--[[
    ...
    Focus:
      - Fluent-inspired polish with Rayfield-style desktop/mobile reliability
...
local function new(className, properties)
    ...
    local objectParent = properties.Parent

    -- Apply Parent last so a control never flashes with Roblox's default selection/style.
    ...
    -- Roblox's default gamepad selection adornment is rectangular and ignores UICorner.
    -- Keep it off unless a future component explicitly opts back into gamepad selection.
    if object:IsA("GuiButton") and properties.Selectable == nil then
...
-- Unified pointer helpers. The visual controls can stay compact while their
-- invisible hit areas remain comfortable on phones/tablets.
...
local function findScrollingAncestor(instance)
...
local function setAncestorScrolling(instance, enabled)
    local scrolling = findScrollingAncestor(instance)
    if not scrolling then
        return nil
    end
    local previous = scrolling.ScrollingEnabled
    scrolling.ScrollingEnabled = enabled
    return scrolling, previous
end
...
local function makeDraggable(handle, target, clampToViewport)
    ...
    local function stopDrag(input)
        ...
        if activeInput and input and activeInput.UserInputType == Enum.UserInputType.Touch and input ~= activeInput then
            return
        end
        dragging = false
        activeInput = nil
    ...
    connect(UserInputService.InputChanged, function(input)
        if not dragging or not dragStart or not startAbsolute then return end
        ...
        local targetAbsolute = startAbsolute + Vector2.new(delta.X, delta.Y)
...
function SectionMethods:CreateButton(options)
    options = options or {}
    local frame = createElementShell(self.Container, options.Description and 58 or 48)
    local title = addDescription(frame, options.Name or "Button", options.Description)
    title.Size = UDim2.new(1, -58, title.Size.Y.Scale, title.Size.Y.Offset)
    ...
    PcdHub:_BindTheme(chevron, "TextColor3", "MutedText")

    -- Full-card hitbox: much easier to press on mobile than a tiny arrow button.
    ...
    connect(hitbox.InputBegan, function(input)
        if isPrimaryPointer(input) and not pressed then
            pressed = true
            tween(frame, 0.08, {BackgroundTransparency = 0})
            tween(chevron, 0.08, {Position = UDim2.new(1, -11, 0.5, 0)})
        end
    end)
    connect(hitbox.InputEnded, function(input)
        if isPrimaryPointer(input) then
            pressed = false
            tween(frame, 0.12, {BackgroundTransparency = 0.04})
            tween(chevron, 0.12, {Position = UDim2.new(1, -14, 0.5, 0)})
        end
    end)
...
function SectionMethods:CreateToggle(options)
    ...
    title.Size = UDim2.new(1, -78, title.Size.Y.Scale, title.Size.Y.Offset)

    local switch = new("Frame", {
        AnchorPoint = Vector2.new(1, 0.5),
        Position = UDim2.new(1, -13, 0.5, 0),
        Size = UDim2.fromOffset(40, 22),
        BackgroundColor3 = current and theme().Accent or theme().Background,
        BorderSizePixel = 0,
        ZIndex = 3,
        Parent = frame,
    })
    corner(switch, 20)
    stroke(switch, "Border", current and 1 or 0.25, 1)

    local knob = new("Frame", {
        AnchorPoint = Vector2.new(0, 0.5),
        Position = current and UDim2.new(0, 20, 0.5, 0) or UDim2.new(0, 3, 0.5, 0),
    ...
    local function set(value, fromConfig)
        current = value == true
        if flag then PcdHub.Flags[flag] = current end
        render(current)
    ...
    local handle = {}
    function handle:Set(value) set(value, false) end
...
function SectionMethods:CreateSlider(options)
    ...
    local flag = resolveFlag(options)
    local touchFriendly = UserInputService.TouchEnabled

    local frame = createElementShell(self.Container, options.Description and 82 or 72)
    local title = addDescription(frame, options.Name or "Slider", options.Description)
    title.Size = UDim2.new(1, -94, 0, 22)
    ...
    corner(valuePill, 7)
    stroke(valuePill, "Border", 0.35, 1)
    PcdHub:_BindTheme(valuePill, "BackgroundColor3", "Background")
    ...
    corner(bar, 10)
    PcdHub:_BindTheme(bar, "BackgroundColor3", "Background")
    ...
    local knob = new("Frame", {
        ...
        Size = UDim2.fromOffset(touchFriendly and 18 or 16, touchFriendly and 18 or 16),
    ...
    corner(knob, 20)
    stroke(knob, "Accent", 0.05, 2)
    PcdHub:_BindTheme(knob, "BackgroundColor3", "Text")

    -- Visual rail stays slim; input area is intentionally much taller.
    local hitboxHeight = touchFriendly and TOUCH_TARGET or 30
    local hitbox = new("TextButton", {
        AnchorPoint = Vector2.new(0, 0.5),
        Position = UDim2.new(0, 0, 0.5, 0),
        Size = UDim2.new(1, 0, 0, hitboxHeight),
        BackgroundTransparency = 1,
        Text = "",
        AutoButtonColor = false,
        ZIndex = 5,
    ...
    local dragging = false
    local activeInput = nil
    local scrollFrame, oldScrollState
    ...
    local function render(value, immediate)
        ...
        if immediate then
            ...
            tween(fill, 0.12, {Size = fillSize})
            tween(knob, 0.12, {Position = knobPos})
        end
        valueLabel.Text = tostring(value) .. suffix
    end

    local function set(value, fromConfig, immediate)
        value = quantize(tonumber(value) or minimum)
        local changed = value ~= current
        current = value
        if flag then PcdHub.Flags[flag] = current end
        render(current, immediate == true)

        if (changed or options.FireOnSameValue == true) and (not fromConfig or options.FireOnLoad == true) then
            safeCall(options.Callback, current)
        end
    end
    ...
    local function setFromPosition(position)
        ...
        local relative = math.clamp((position.X - bar.AbsolutePosition.X) / width, 0, 1)
        set(minimum + (maximum - minimum) * relative, false, true)
    end

    local function endDrag(input)
        if not dragging then return end
        if activeInput and activeInput.UserInputType == Enum.UserInputType.Touch and input and input ~= activeInput then
            return
        end
        dragging = false
        activeInput = nil
        tween(knob, 0.12, {Size = UDim2.fromOffset(touchFriendly and 18 or 16, touchFriendly and 18 or 16)})
        if scrollFrame then
            scrollFrame.ScrollingEnabled = oldScrollState ~= false
        end
        scrollFrame, oldScrollState = nil, nil
    end
    ...
    connect(hitbox.InputBegan, function(input)
        ...
        dragging = true
        activeInput = input
        scrollFrame, oldScrollState = setAncestorScrolling(frame, false)
        setFromPosition(input.Position)
        tween(knob, 0.1, {Size = UDim2.fromOffset(touchFriendly and 22 or 19, touchFriendly and 22 or 19)})
    ...
    connect(UserInputService.InputEnded, function(input)
        if isPrimaryPointer(input) then endDrag(input) end
    end)
    ...
    local handle = {}
    function handle:Set(value) set(value, false, false) end
    function handle:Get() return current end
    return handle
end
...
function SectionMethods:CreateInput(options)
    ...
    local touchFriendly = UserInputService.TouchEnabled
    local frame = createElementShell(self.Container, options.Description and (touchFriendly and 78 or 72) or (touchFriendly and 68 or 62))
    local title = addDescription(frame, options.Name or "Input", options.Description)
    title.Size = UDim2.new(1, -28, 0, 20)

    local input = new("TextBox", {
        Position = UDim2.new(0, 14, 1, touchFriendly and -40 or -32),
        Size = UDim2.new(1, -28, 0, touchFriendly and 32 or 24),
        BackgroundColor3 = theme().Background,
        BorderSizePixel = 0,
...
function SectionMethods:CreateColorPicker(options)
    ...
    local CLOSED_HEIGHT = 48
    local PANEL_Y = 54
    local PANEL_HEIGHT = UserInputService.TouchEnabled and 164 or 148
    local OPEN_HEIGHT = PANEL_Y + PANEL_HEIGHT + 8

    local frame = createElementShell(self.Container, CLOSED_HEIGHT)
    local title = addDescription(frame, options.Name or "Color Picker", options.Description)
    title.Position = UDim2.new(0, 14, 0, 0)
    title.Size = UDim2.new(1, -84, 0, CLOSED_HEIGHT)

    local preview = new("TextButton", {
        AnchorPoint = Vector2.new(1, 0),
        Position = UDim2.new(1, -10, 0, UserInputService.TouchEnabled and 6 or 11),
        Size = UDim2.new(0, UserInputService.TouchEnabled and 58 or 46, 0, UserInputService.TouchEnabled and 36 or 26),
    ...
    local panel = new("Frame", {
        Position = UDim2.new(0, 8, 0, PANEL_Y),
    ...
    local sv = new("ImageButton", {
        ...
        Size = UDim2.new(1, UserInputService.TouchEnabled and -60 or -48, 0, UserInputService.TouchEnabled and 118 or 105),
    ...
    local hue = new("ImageButton", {
        ...
        Size = UDim2.new(0, UserInputService.TouchEnabled and 30 or 24, 0, UserInputService.TouchEnabled and 118 or 105),
    ...
    local hueHitbox = new("TextButton", {
        ...
        Size = UDim2.new(0, UserInputService.TouchEnabled and TOUCH_TARGET or 30, 1, UserInputService.TouchEnabled and 12 or 4),
        BackgroundTransparency = 1,
        Text = "",
        AutoButtonColor = false,
        ZIndex = 34,
    ...
    setOpen = function(state)
        ...
        tween(panel, 0.16, {Size = UDim2.new(1, -16, 0, open and PANEL_HEIGHT or 0)})
        tween(frame, 0.16, {Size = UDim2.new(1, 0, 0, open and OPEN_HEIGHT or CLOSED_HEIGHT)})
    ...
    local function beginColorDrag(kind, input)
        if not isPrimaryPointer(input) then return end
        scrollFrame, oldScrollState = setAncestorScrolling(frame, false)
        if kind == "sv" then
            svDragging = true
        ...
        connect(input.Changed, function()
            if input.UserInputState == Enum.UserInputState.End then
                if kind == "sv" and svInput == input then
                    svDragging, svInput = false, nil
                elseif kind == "hue" and hueInput == input then
                    hueDragging, hueInput = false, nil
                end
                if not svDragging and not hueDragging and scrollFrame then
                    scrollFrame.ScrollingEnabled = oldScrollState ~= false
                    scrollFrame, oldScrollState = nil, nil
                end
            end
    ...
    connect(sv.InputBegan, function(input)
        beginColorDrag("sv", input)
    end)
    connect(hueHitbox.InputBegan, function(input)
        beginColorDrag("hue", input)
    end)
    connect(UserInputService.InputChanged, function(input)
        if svDragging then
            if svInput and svInput.UserInputType == Enum.UserInputType.Touch then
                if input == svInput then updateSV(input) end
            elseif input.UserInputType == Enum.UserInputType.MouseMovement then
                updateSV(input)
            end
        end
        if hueDragging then
            if hueInput and hueInput.UserInputType == Enum.UserInputType.Touch then
                if input == hueInput then updateHue(input) end
            elseif input.UserInputType == Enum.UserInputType.MouseMovement then
                updateHue(input)
            end
        end
    ...
    connect(UserInputService.InputEnded, function(input)
        if svInput == input or (svInput and svInput.UserInputType == Enum.UserInputType.MouseButton1 and input.UserInputType == Enum.UserInputType.MouseButton1) then
            svDragging, svInput = false, nil
        end
        if hueInput == input or (hueInput and hueInput.UserInputType == Enum.UserInputType.MouseButton1 and input.UserInputType == Enum.UserInputType.MouseButton1) then
            hueDragging, hueInput = false, nil
        end
        if not svDragging and not hueDragging and scrollFrame then
            scrollFrame.ScrollingEnabled = oldScrollState ~= false
...
function SectionMethods:CreateDropdown(options)
    ...
    if multi then
        ...
        selected = type(initial) == "table" and initial[1] or initial
    end

    local touchFriendly = UserInputService.TouchEnabled
    local CLOSED_HEIGHT = touchFriendly and 52 or 48
    local LIST_Y = CLOSED_HEIGHT + 6
    local OPTION_HEIGHT = touchFriendly and 40 or 32
    local OPTION_GAP = 4
    local MAX_LIST_HEIGHT = touchFriendly and 220 or 180

    local frame = createElementShell(self.Container, CLOSED_HEIGHT)
    local title = addDescription(frame, options.Name or "Dropdown", nil)
    title.Position = UDim2.new(0, 14, 0, 0)
    title.Size = UDim2.new(0.43, -14, 0, CLOSED_HEIGHT)

    local dropdownButton = new("TextButton", {
        AnchorPoint = Vector2.new(1, 0),
        Position = UDim2.new(1, -10, 0, touchFriendly and 8 or 9),
        Size = UDim2.new(0.52, 0, 0, touchFriendly and 36 or 30),
        BackgroundColor3 = theme().Background,
        BorderSizePixel = 0,
        Font = Enum.Font.Gotham,
        Text = "Select",
    ...
    padding(dropdownButton, 9, 26, 0, 0)
    ...
    PcdHub:_BindTheme(arrow, "TextColor3", "MutedText")
    ...
    corner(list, 9)
    stroke(list, "Border", 0.08, 1)
    PcdHub:_BindTheme(list, "BackgroundColor3", "Surface")
    PcdHub:_BindTheme(list, "ScrollBarImageColor3", "Accent")
    ...
    local function refreshVisual()
        ...
        for value, data in pairs(optionButtons) do
            local active = multi and selected[value] == true or tostring(selected) == value
            data.Button.BackgroundColor3 = active and theme().AccentSoft or theme().Background
            data.Button.TextColor3 = active and theme().Text or theme().MutedText
    ...
    local function set(value, fromConfig)
        if multi then
            ...
            selected = type(value) == "table" and value[1] or value
        end
    ...
    local function rebuild()
        ...
        for _, item in ipairs(items) do
            ...
            local value = tostring(itemValue)
            ...
            PcdHub:_BindTheme(check, "TextColor3", "Accent")

            optionButtons[value] = {Button = button, Check = check}
    ...
    setOpen = function(state)
        open = state == true
        local contentHeight = #items * (OPTION_HEIGHT + OPTION_GAP) + 12
        local wanted = math.min(contentHeight, MAX_LIST_HEIGHT)
        local openHeight = LIST_Y + wanted + 8
        ...
        list.ScrollBarImageTransparency = contentHeight > MAX_LIST_HEIGHT and 0 or 1
        tween(arrow, 0.14, {Rotation = open and 180 or 0})
        tween(list, 0.16, {Size = UDim2.new(1, -16, 0, open and wanted or 0)})
        tween(frame, 0.16, {Size = UDim2.new(1, 0, 0, open and openHeight or CLOSED_HEIGHT)})
    ...
    function handle:SetOpen(state)
        if state == nil then state = not open end
        if state ~= open then setOpen(state) end
    end
...
function SectionMethods:CreateKeybind(options)
    ...
    local touchFriendly = UserInputService.TouchEnabled
    local frame = createElementShell(self.Container, touchFriendly and 52 or 46)
    ...
    local button = new("TextButton", {
        ...
        Size = UDim2.new(0, touchFriendly and 104 or 92, 0, touchFriendly and 34 or 28),
        BackgroundColor3 = theme().Background,
        BorderSizePixel = 0,
        Font = Enum.Font.GothamSemibold,
        Text = current.Name,
    ...
    connect(button.Activated, function()
        if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled then
            ...
            task.delay(0.8, function()
                if button.Parent and not listening then
                    button.Text = current.Name
                end
            end)
            return
...
function PcdHub:CreateWindow(options)
    ...
    local requestedWidth = tonumber(options.Width) or 570
    local requestedHeight = tonumber(options.Height) or 360
    ...
    local compactBreakpoint = 620
    local compact = viewport.X <= compactBreakpoint
    local sidebarExpandedWidth = compact and 124 or 146
    ...
    local headerHeight = UserInputService.TouchEnabled and 50 or 46
    ...
    self:_BindTheme(headerDivider, "BackgroundColor3", "Border")

    local menuButton = new("TextButton", {
        Position = UDim2.new(0, 8, 0.5, UserInputService.TouchEnabled and -17 or -14),
        Size = UDim2.fromOffset(UserInputService.TouchEnabled and 34 or 28, UserInputService.TouchEnabled and 34 or 28),
        BackgroundColor3 = theme().SurfaceAlt,
        BorderSizePixel = 0,
        Font = Enum.Font.GothamBold,
        Text = "≡",
    ...
    local title = new("TextLabel", {
        BackgroundTransparency = 1,
        Position = UDim2.new(0, UserInputService.TouchEnabled and 50 or 44, 0, 6),
    ...
    local subtitle = new("TextLabel", {
        BackgroundTransparency = 1,
        Position = UDim2.new(0, UserInputService.TouchEnabled and 50 or 44, 0, UserInputService.TouchEnabled and 27 or 24),
    ...
    local minimize = new("TextButton", {
        AnchorPoint = Vector2.new(1, 0.5),
        Position = UDim2.new(1, UserInputService.TouchEnabled and -46 or -40, 0.5, 0),
        Size = UDim2.fromOffset(UserInputService.TouchEnabled and 34 or 28, UserInputService.TouchEnabled and 34 or 28),
        BackgroundColor3 = theme().SurfaceAlt,
        BorderSizePixel = 0,
        Font = Enum.Font.GothamBold,
    ...
    local searchBox = new("TextBox", {
        ...
        Size = UDim2.new(1, -16, 0, UserInputService.TouchEnabled and 34 or 30),
        BackgroundColor3 = theme().SurfaceAlt,
        BorderSizePixel = 0,
        ClearTextOnFocus = false,
        Font = Enum.Font.Gotham,
    ...
    local tabHolder = new("ScrollingFrame", {
        Position = UDim2.new(0, 7, 0, sidebarCollapsed and 8 or (UserInputService.TouchEnabled and 50 or 46)),
        Size = UDim2.new(1, -14, 1, sidebarCollapsed and -16 or (UserInputService.TouchEnabled and -58 or -54)),
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        ScrollBarThickness = 2,
        ScrollBarImageColor3 = theme().Accent,
    ...
    local function renderSidebar(animated)
        ...
        tabHolder.Position = UDim2.new(0, 7, 0, sidebarCollapsed and 8 or (UserInputService.TouchEnabled and 50 or 46))
        tabHolder.Size = UDim2.new(1, -14, 1, sidebarCollapsed and -16 or (UserInputService.TouchEnabled and -58 or -54))
    ...
    local function selectTab(tab)
        ...
        if tab and tab.Page then
            tab.Page.Position = UDim2.new(0, 8, 0, 8)
            tween(tab.Page, 0.18, {Position = UDim2.new(0, 10, 0, 8)})
        end
        searchBox.Text = ""
        applySearch("")
    end
    ...
    function window:CreateTab(name, icon)
        ...
        local button = new("TextButton", {
            Size = UDim2.new(1, 0, 0, UserInputService.TouchEnabled and 42 or 36),
            BackgroundColor3 = theme().SurfaceAlt,
            BorderSizePixel = 0,
            Text = "",
            AutoButtonColor = false,
    ...
    if camera then
        connect(camera:GetPropertyChangedSignal("ViewportSize"), function()
            ...
            sidebarExpandedWidth = compact and 124 or 146
