-- AstraUI V5.4.1 - Adaptive Layout Engine / Interaction Stability
...
function Window:_computeAdaptiveState()
    ...
    local compact=(w<640 or h<330)
    local veryCompact=(w<540 or h<285)
    if not self.AdaptiveMode then compact=false;veryCompact=false end
    local forceCollapsed=self.AdaptiveMode and (portrait or (mobile and w<760) or w<610) or false
    local openSidebar=(w>=900 and 176) or (w>=760 and 168) or 160
    local sidebar=forceCollapsed and 50 or openSidebar
    if self._manualSidebar~=nil and not forceCollapsed then sidebar=self._manualSidebar and 50 or openSidebar end
    local collapsed=sidebar<=56
    local contentWidth=math.max(0,w-sidebar)
    local stackActions=self.AdaptiveMode and (portrait or contentWidth<430) or false
    local hideDescriptions=self.AdaptiveMode and (veryCompact or (mobile and contentWidth<470)) or false
    local hidePageSubtitle=self.AdaptiveMode and (contentWidth<520 or h<320) or false
    local pageGap=veryCompact and 7 or (compact and 9 or 12)
    local pagePad=veryCompact and 9 or (compact and 11 or 14)
    local topbar=veryCompact and 54 or 58
    local state={
        Width=w,Height=h,ContentWidth=contentWidth,Mobile=mobile,Portrait=portrait,
        ...
        PageGap=pageGap,PagePadding=pagePad,TopbarHeight=topbar,Layout=d.Layout,
    }
...
function Window:_responsive()
    ...
    self.SearchHolder.Visible=not state.SidebarCollapsed and not state.VeryCompact
    self.BrandTitle.Visible=not state.SidebarCollapsed
    self.BrandSubtitle.Visible=not state.SidebarCollapsed and not state.Compact
    self.Footer.Visible=not state.SidebarCollapsed and not state.VeryCompact
    ...
    self.PageTitle.Position=UDim2.fromOffset(18,state.HidePageSubtitle and 16 or 8)
    self.PageTitle.Size=UDim2.new(1,-170,0,22)
    self.PageSubtitle.Position=UDim2.fromOffset(18,31)

    local pagePadding, pageList
    if self.SelectedTab and self.SelectedTab.Page then
        pagePadding=self.SelectedTab.Page:FindFirstChildOfClass("UIPadding")
        pageList=self.SelectedTab.Page:FindFirstChildOfClass("UIListLayout")
    end
...
local function preset(w,name)
    ...
    if w:IsMobile() then
        if d.Orientation=="Portrait" then
            p={
                small=Vector2.new(math.min(300,sm.X),math.min(430,sm.Y)),
                normal=Vector2.new(math.min(350,sm.X),math.min(560,sm.Y)),
                large=Vector2.new(math.min(390,sm.X),math.min(650,sm.Y)),
                max=sm,
            }
        else
            local phone=d.FormFactor=="Phone"
            p={
                small=Vector2.new(math.min(phone and 520 or 620,sm.X),math.min(phone and 300 or 360,sm.Y)),
                normal=Vector2.new(math.min(phone and 680 or 760,sm.X),math.min(phone and 360 or 430,sm.Y)),
                large=Vector2.new(math.min(phone and 800 or 900,sm.X),math.min(phone and 430 or 520,sm.Y)),
                max=sm,
            }
        end
    else
...
function Window:SetScale(v)
    local lo,hi=.80,1.25
    if self:IsMobile() then lo,hi=.95,1.15 end
...
function Window:Notify(o)
    ...
    if duration==nil then duration=4.5 end
    ...
    local targetWidth=mobile and 264 or 304
    local width=math.max(210,math.min(targetWidth,v.X-20))
    local ch=content~="" and textHeight(content,10,width-50) or 0
    local height=math.floor(clamp(42+(ch>0 and math.min(ch,30)+7 or 0),50,mobile and 78 or 84))
    local now=os.clock()
    local groupKey=tostring(o.GroupKey or (typ.."|"..title.."|"..content))
    local dedupeWindow=math.max(0,tonumber(o.DeduplicateWindow) or .65)

    -- Collapse accidental duplicate notifications (common with touch callbacks).
    ...
    -- Never allow toasts to cover half the interface on mobile.
    local maxVisible=math.max(1,math.floor(tonumber(o.MaxVisible) or tonumber(self.MaxVisibleNotifications) or (mobile and 3 or 4)))
    local active={}
    for _,n in ipairs(self.Notifications) do
        if n and not n.Closed then table.insert(active,n) end
    end
    while #active>=maxVisible do
        local oldest=table.remove(active,1)
        if oldest and oldest.Close then oldest.Close(true) end
    end

    self.NotificationRoot.Size=UDim2.fromOffset(math.max(width,1),1)
    self.NotificationRoot.Position=UDim2.new(1,-12,0,12)
    ...
    local f=create("Frame",{
        ...
        Position=UDim2.new(1,width+14,0,0),
    ...
    f:SetAttribute("AstraBaseRadius",12)
    corner(f,12)
    stroke(f,t.BorderSubtle,.18,1)

    local stripe=create("Frame",{Parent=f,Size=UDim2.new(0,3,1,0),BackgroundColor3=tone,BorderSizePixel=0,ZIndex=1102})
    corner(stripe,2)

    local tt=label(f,title,11,t.TextPrimary,Enum.Font.GothamBold)
    tt.Position=UDim2.fromOffset(13,7)
    tt.Size=UDim2.new(1,-44,0,17)
    tt.TextTruncate=Enum.TextTruncate.AtEnd
    tt.ZIndex=1103

    local closeButton=create("TextButton",{
        Parent=f,AnchorPoint=Vector2.new(1,0),Position=UDim2.new(1,-6,0,5),
        Size=UDim2.fromOffset(26,26),BackgroundTransparency=1,Text="×",
        TextColor3=t.TextSecondary,TextSize=15,Font=Enum.Font.GothamMedium,
        BorderSizePixel=0,ZIndex=1104,AutoButtonColor=false,
    })

    local body=label(f,content,9,t.TextSecondary)
    body.Position=UDim2.fromOffset(13,25)
    body.Size=UDim2.new(1,-26,0,math.max(15,math.min(ch,30)))
    ...
    local progress=create("Frame",{
        Parent=f,AnchorPoint=Vector2.new(0,1),Position=UDim2.new(0,10,1,-5),
        Size=UDim2.new(1,-20,0,2),BackgroundColor3=tone,
        BackgroundTransparency=persistent and 1 or 0,BorderSizePixel=0,ZIndex=1103,
    })
    corner(progress,2)
    ...
    table.insert(self.Notifications,rec)
    table.insert(self.NotificationHistory,rec)

    local function close(fast)
        if rec.Closed then return end
        rec.Closed=true
        local exitTime=fast and .10 or .18
        tween(sc,fast and .07 or .12,{Scale=.96},Enum.EasingStyle.Quad,Enum.EasingDirection.In)
        tween(f,exitTime,{Position=UDim2.new(1,width+14,0,0),BackgroundTransparency=.12},Enum.EasingStyle.Quart,Enum.EasingDirection.In)
        tween(tt,fast and .06 or .10,{TextTransparency=1})
        tween(body,fast and .06 or .10,{TextTransparency=1})
        tween(closeButton,fast and .06 or .10,{TextTransparency=1})
        tween(stripe,fast and .06 or .10,{BackgroundTransparency=1})
        tween(progress,fast and .06 or .10,{BackgroundTransparency=1})
        task.delay(exitTime+.03,function()
            local i=table.find(self.Notifications,rec)
    ...
    tween(sc,.18,{Scale=1},Enum.EasingStyle.Back,Enum.EasingDirection.Out)
    tween(f,.20,{Position=UDim2.new(1,0,0,0)},Enum.EasingStyle.Quart,Enum.EasingDirection.Out)

    if not persistent then
        tween(progress,duration,{Size=UDim2.new(0,0,0,2)},Enum.EasingStyle.Linear,Enum.EasingDirection.InOut)
        task.delay(duration,function()
            if not self.Destroyed and not rec.Closed then close(false) end
        end)
    end
...
function Window:GetNotificationLimit()return self.MaxVisibleNotifications or (self:IsMobile() and 3 or 4)end
...
function Tab:_sidebar(c)self.Label.Visible=not c;self.IconHost.Position=c and UDim2.fromOffset(17,13)or UDim2.fromOffset(13,13)end
function Tab:_theme()
...
local function card(s,name,desc,h)
    ...
    local c=setmetatable({Window=w,Section=s,Instance=r,Title=a,Description=d,ActionReserve=0,BaseHeight=h or 60},Control)
    ...
    function c:ApplyAdaptive(state)
        ...
        local hide=state.HideDescriptions and hasDesc and self.BaseHeight<=90
        self.Description.Visible=hasDesc and not hide
        if hide then
            self.Title.Position=UDim2.fromOffset(12,math.max(7,math.floor((self.BaseHeight-19)/2)))
        else
            self.Title.Position=UDim2.fromOffset(12,8)
            self.Description.Position=UDim2.fromOffset(12,28)
        end
...
function Section:AddButton(o)
    o=o or {};local r,c=card(self,o.Name or "Button",o.Description,56);local w=self.Window;local b=button(r,o.ButtonText or o.Text or o.Name or "Run",w.Theme,o.Style or "Primary");b.AnchorPoint=Vector2.new(1,.5);b.Position=UDim2.new(1,-12,.5,0);b.Size=UDim2.fromOffset(clamp(textWidth(b.Text,11)+30,76,138),32);c.Button=b;c:SetActionReserve(b.Size.X.Offset+18)
    c._adaptiveHook=function(self,state)
        if state.StackActions then
            self.Instance.Size=UDim2.new(1,0,0,86);self.BaseHeight=86;self.Description.Visible=not state.HideDescriptions and self.Description.Text~=""
            self.Title.Position=UDim2.fromOffset(12,8);self.Description.Position=UDim2.fromOffset(12,28)
            b.AnchorPoint=Vector2.new(0,1);b.Position=UDim2.new(0,12,1,-10);b.Size=UDim2.new(1,-24,0,32);self:SetActionReserve(0)
        else
            self.BaseHeight=56;self.Instance.Size=UDim2.new(1,0,0,56);b.AnchorPoint=Vector2.new(1,.5);b.Position=UDim2.new(1,-12,.5,0);b.Size=UDim2.fromOffset(clamp(textWidth(b.Text,11)+30,76,138),32);self:SetActionReserve(b.Size.X.Offset+18)
        end
    end
    ...
    w:_connect(b.Activated,function()if c.Disabled or type(o.Callback)~="function"then return end;local old=b.Text;if o.Async then b.Text=o.LoadingText or "Loading...";b.Active=false end;safeCall(o.Callback,c);if o.Async then b.Text=old;b.Active=true end end);if type(o.Callback)~="function"then c:SetDisabled(true)end;return c
end
function Tab:AddButton(o)return self:_defaultSection():AddButton(o)end

function Section:AddToggle(o)
    o=o or {};local def=not not o.Default;local r,c=card(self,o.Name or "Toggle",o.Description,56);local w=self.Window;local tr=create("TextButton",{Parent=r,AnchorPoint=Vector2.new(1,.5),Position=UDim2.new(1,-12,.5,0),Size=UDim2.fromOffset(46,24),BackgroundColor3=w.Theme.Surface3,BorderSizePixel=0,Text="",AutoButtonColor=false});corner(tr,14);local k=create("Frame",{Parent=tr,AnchorPoint=Vector2.new(.5,.5),Position=UDim2.fromOffset(def and 34 or 12,12),Size=UDim2.fromOffset(18,18),BackgroundColor3=def and Color3.new(1,1,1)or w.Theme.TextSecondary,BorderSizePixel=0});corner(k,99);local val=def;c:SetActionReserve(64)
    local function apply(v,fire)val=not not v;tr.BackgroundColor3=val and w.Theme.Accent or w.Theme.Surface3;k.BackgroundColor3=val and Color3.new(1,1,1)or w.Theme.TextSecondary;tween(k,.14,{Position=UDim2.fromOffset(val and 34 or 12,12)});w:_setFlag(o.Flag,val,c);if fire~=false then safeCall(o.Callback,val)end end
    function c:Set(v,fire)apply(v,fire)end;function c:Get()return val end;function c:GetDefault()return def end;function c:SetDisabled(v)self.Disabled=not not v;tr.Active=not self.Disabled;tr.BackgroundTransparency=self.Disabled and .45 or 0 end;flag(c,o,def);w:_connect(tr.Activated,function()if not c.Disabled then apply(not val,true)end end);apply(def,false);return c
end
function Tab:AddToggle(o)return self:_defaultSection():AddToggle(o)end

function Section:AddSlider(o)
    o=o or {};local mn=tonumber(o.Min)or 0;local mx=tonumber(o.Max)or 100;local inc=tonumber(o.Increment)or 1;local def=clamp(tonumber(o.Default)or mn,mn,mx);local r,c=card(self,o.Name or "Slider",o.Description,82);local w=self.Window;local vl=label(r,"",10,w.Theme.Accent,Enum.Font.GothamBold,Enum.TextXAlignment.Right);vl.AnchorPoint=Vector2.new(1,0);vl.Position=UDim2.new(1,-14,0,9);vl.Size=UDim2.fromOffset(80,20);local rail=create("TextButton",{Parent=r,Position=UDim2.fromOffset(14,58),Size=UDim2.new(1,-28,0,4),BackgroundColor3=w.Theme.Surface3,BorderSizePixel=0,Text="",AutoButtonColor=false});corner(rail,4);local fill=create("Frame",{Parent=rail,Size=UDim2.new(0,0,1,0),BackgroundColor3=w.Theme.Accent,BorderSizePixel=0});corner(fill,4);local knob=create("Frame",{Parent=rail,AnchorPoint=Vector2.new(.5,.5),Position=UDim2.new(0,0,.5,0),Size=UDim2.fromOffset(16,16),BackgroundColor3=Color3.new(1,1,1),BorderSizePixel=0});corner(knob,99);stroke(knob,w.Theme.Accent,0,2);local val=def
    local function norm(v)v=clamp(v,mn,mx);v=math.floor((v-mn)/inc+.5)*inc+mn;return clamp(v,mn,mx)end
    local function apply(v,fire)val=norm(tonumber(v)or mn);local a=mx==mn and 0 or (val-mn)/(mx-mn);fill.Size=UDim2.new(a,0,1,0);knob.Position=UDim2.new(a,0,.5,0);vl.Text=tostring(val)..tostring(o.Suffix or "");w:_setFlag(o.Flag,val,c);if fire~=false then safeCall(o.Callback,val)end end
    ...
    w:_connect(rail.InputBegan,function(i)
    ...
    w:_connect(UserInputService.InputChanged,function(i)
        ...
        if activeTouch then
            if i==activeTouch then fromX(i.Position.X) end
    ...
    w:_connect(UserInputService.InputEnded,function(i)
        ...
        if released then
            drag=false;activeTouch=nil
            if o.CallbackOnRelease then safeCall(o.CallbackOnRelease,val) end
        end
    end)
    function c:Set(v,fire)apply(v,fire)end;function c:Get()return val end;function c:GetDefault()return def end;function c:SetDisabled(v)self.Disabled=not not v;rail.Active=not self.Disabled;r.BackgroundTransparency=self.Disabled and .3 or 0 end;flag(c,o,def);apply(def,false);return c
end
function Tab:AddSlider(o)return self:_defaultSection():AddSlider(o)end

function Section:AddInput(o)
    o=o or {};local def=tostring(o.Default or "");local r,c=card(self,o.Name or "Input",o.Description,60);local w=self.Window;local box=create("TextBox",{Parent=r,AnchorPoint=Vector2.new(1,.5),Position=UDim2.new(1,-12,.5,0),Size=UDim2.fromOffset(200,34),BackgroundColor3=w.Theme.Surface3,BorderSizePixel=0,Text=def,PlaceholderText=tostring(o.Placeholder or "Enter value..."),TextColor3=w.Theme.TextPrimary,PlaceholderColor3=w.Theme.TextSecondary,TextSize=10,Font=Enum.Font.Gotham,ClearTextOnFocus=false,TextXAlignment=Enum.TextXAlignment.Left});corner(box,8);stroke(box,w.Theme.Border,.35,1);padding(box,10,0,10,0);c.Box=box;c:SetActionReserve(226)
    c._adaptiveHook=function(self,state)
        if state.StackActions then
            self.BaseHeight=96;self.Instance.Size=UDim2.new(1,0,0,96);self.Title.Position=UDim2.fromOffset(12,8);self.Description.Position=UDim2.fromOffset(12,28);self.Description.Visible=not state.HideDescriptions and self.Description.Text~="";box.AnchorPoint=Vector2.new(0,1);box.Position=UDim2.new(0,12,1,-10);box.Size=UDim2.new(1,-24,0,34);self:SetActionReserve(0)
        else
            self.BaseHeight=60;self.Instance.Size=UDim2.new(1,0,0,60);box.AnchorPoint=Vector2.new(1,.5);box.Position=UDim2.new(1,-12,.5,0);box.Size=UDim2.fromOffset(200,34);self:SetActionReserve(226)
        end
    end
    function c:Set(v,fire)box.Text=tostring(v or "");w:_setFlag(o.Flag,box.Text,c);if fire~=false then safeCall(o.Callback,box.Text)end end;function c:Get()return box.Text end;function c:GetDefault()return def end;function c:SetDisabled(v)self.Disabled=not not v;box.TextEditable=not self.Disabled;box.BackgroundTransparency=self.Disabled and .35 or 0 end;flag(c,o,def);w:_connect(box.Focused,function()stroke(box,w.Theme.Accent,0,1.5)end);w:_connect(box.FocusLost,function(enter)stroke(box,w.Theme.Border,.25,1);w:_setFlag(o.Flag,box.Text,c);if not o.CallbackOnEnter or enter then safeCall(o.Callback,box.Text,enter)end end);return c
end
...
function Window:_dropdownPopover(o,b,choose)
    ...
    self._popover=layer;self:_connect(catcher.Activated,function()if layer.Parent then layer:Destroy()end;if self._popover==layer then self._popover=nil end end)
    for _,v0 in ipairs(vals)do local ib=button(sf,tostring(v0),self.Theme,"Ghost");ib.Size=UDim2.new(1,-2,0,30);ib.ZIndex=1202;self:_connect(ib.Activated,function()choose(v0);if not o.Multi and layer.Parent then layer:Destroy();self._popover=nil end end)end
    return pop
end
function Section:AddDropdown(o)
    o=o or {};local multi=o.Multi==true;local def=o.Default;if multi and type(def)~="table"then def={}elseif not multi and def==nil then def=(o.Options or {})[1]end;local r,c=card(self,o.Name or "Dropdown",o.Description,60);local w=self.Window;local b=button(r,"",w.Theme,"Secondary");b.AnchorPoint=Vector2.new(1,.5);b.Position=UDim2.new(1,-12,.5,0);b.Size=UDim2.fromOffset(200,34);local val=multi and copy(def)or def;c:SetActionReserve(226)
    c._adaptiveHook=function(self,state)
        if state.StackActions then
            self.BaseHeight=96;self.Instance.Size=UDim2.new(1,0,0,96);self.Title.Position=UDim2.fromOffset(12,8);self.Description.Position=UDim2.fromOffset(12,28);self.Description.Visible=not state.HideDescriptions and self.Description.Text~="";b.AnchorPoint=Vector2.new(0,1);b.Position=UDim2.new(0,12,1,-10);b.Size=UDim2.new(1,-24,0,34);self:SetActionReserve(0)
        else
            self.BaseHeight=60;self.Instance.Size=UDim2.new(1,0,0,60);b.AnchorPoint=Vector2.new(1,.5);b.Position=UDim2.new(1,-12,.5,0);b.Size=UDim2.fromOffset(200,34);self:SetActionReserve(226)
        end
    end
    ...
    local function choose(v)if multi then local n=copy(val);n[v]=not n[v];apply(n,true)else apply(v,true)end end;w:_connect(b.Activated,function()if not c.Disabled then w:_dropdownPopover(o,b,choose)end end);function c:Set(v,fire)apply(v,fire)end;function c:Get()return val end;function c:GetDefault()return def end;function c:SetOptions(v)o.Options=v or {}end;function c:SetDisabled(v)self.Disabled=not not v;b.Active=not self.Disabled;b.BackgroundTransparency=self.Disabled and .35 or 0 end;flag(c,o,def);apply(def,false);return c
end
...
function Section:AddQuickActions(o)
    ...
    local r,c=rich(self,o.Name or "Quick Actions",o.Description or "Frequently used actions",118)
    local host=create("Frame",{Parent=r,Position=UDim2.fromOffset(14,54),Size=UDim2.new(1,-28,0,46),BackgroundTransparency=1})
    local g=Instance.new("UIGridLayout");g.CellPadding=UDim2.fromOffset(8,8);g.SortOrder=Enum.SortOrder.LayoutOrder;g.Parent=host
    ...
    c._adaptiveHook=function(self,state)
        ...
        local cols=(cw>=680 and 4)or(cw>=520 and 3)or(cw>=340 and 2)or 1
        cols=math.max(1,math.min(cols,math.max(1,#acts)))
        local rowH=36;local gap=8;local rows=math.max(1,math.ceil(math.max(1,#acts)/cols));local hostH=rows*rowH+(rows-1)*gap
        g.FillDirectionMaxCells=cols;g.CellSize=UDim2.new(1/cols,-((cols-1)*gap)/cols,0,rowH);host.Size=UDim2.new(1,-28,0,hostH)
        self.BaseHeight=68+hostH;self.Instance.Size=UDim2.new(1,0,0,self.BaseHeight)
    end
...
function Section:AddSegmentedControl(o)
    ...
    for _,x in ipairs(opts)do local b=button(host,tostring(x),self.Window.Theme,"Ghost");b.Size=UDim2.new(1/math.max(1,#opts),-4,1,0);bs[x]=b;self.Window:_connect(b.Activated,function()apply(x,true)end)end;function c:Set(v,fire)apply(v,fire)end;function c:Get()return val end;flag(c,o,def);apply(def,false);return c
end
...
function Section:AddPlayerProfile(o)
    ...
    local host=create("Frame",{Parent=r,Position=UDim2.fromOffset(16,128),Size=UDim2.new(1,-32,0,70),BackgroundTransparency=1});local g=Instance.new("UIGridLayout");g.CellPadding=UDim2.fromOffset(8,8);g.Parent=host;local refs={};for _,x in ipairs({"PLAYTIME","ACCESS","REMAINING","ACCOUNT"})do local f=create("Frame",{Parent=host,BackgroundColor3=w.Theme.Surface3,BorderSizePixel=0});corner(f,9);local a=label(f,x,8,w.Theme.TextSecondary,Enum.Font.GothamBold);a.Position=UDim2.fromOffset(9,6);a.Size=UDim2.new(1,-18,0,14);local b=label(f,"-",11,w.Theme.TextPrimary,Enum.Font.GothamBold);b.Position=UDim2.fromOffset(9,30);b.Size=UDim2.new(1,-18,0,20);refs[x]=b end
    c._adaptiveHook=function(self,state)
        local narrow=state.ContentWidth<590
        local cols=narrow and 2 or 4;local rows=math.ceil(4/cols);local cellH=62;local gap=8;local hh=rows*cellH+(rows-1)*gap
        g.FillDirectionMaxCells=cols;g.CellSize=UDim2.new(1/cols,-((cols-1)*gap)/cols,0,cellH);host.Size=UDim2.new(1,-32,0,hh)
        if state.ContentWidth<430 then
            av.Size=UDim2.fromOffset(52,52);n.Position=UDim2.fromOffset(80,52);u.Position=UDim2.fromOffset(80,76);ag.Visible=false;st.Size=UDim2.fromOffset(88,26);st.Position=UDim2.new(1,-14,0,54)
        else
            av.Size=UDim2.fromOffset(62,62);n.Position=UDim2.fromOffset(92,52);u.Position=UDim2.fromOffset(92,78);ag.Visible=true;st.Size=UDim2.fromOffset(112,30);st.Position=UDim2.new(1,-16,0,54)
        end
        self.BaseHeight=150+hh;self.Instance.Size=UDim2.new(1,0,0,self.BaseHeight)
    end
