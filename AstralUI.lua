-- AstraUI V5.0.1 - Clean Rebuild Compile Fix
...
local function safeMax(w)
    local v=viewport();local mx=w:IsMobile()and 20 or 50;local my=w:IsMobile()and 34 or 60;return Vector2.new(math.floor((v.X-mx)/w.Scale),math.floor((v.Y-my)/w.Scale))
end
...
function Window:SetScale(v)self.Scale=clamp(tonumber(v)or 1,.65,1.45);self.ScaleObject.Scale=self.Scale;self:SetWindowSize(self.CurrentSize.X,self.CurrentSize.Y,false);return self.Scale end
function Window:SetInterfaceScale(v)return self:SetScale(v)end
function Window:SetUIScale(v)return self:SetScale(v)end
function Window:SetWindowCornerRadius(v)self.WindowCornerRadius=clamp(tonumber(v)or 14,0,28);corner(self.Main,self.WindowCornerRadius);return self.WindowCornerRadius end
...
function Window:Notify(o)
    ...
    local prog=create("Frame",{Parent=f,AnchorPoint=Vector2.new(0,1),Position=UDim2.new(0,12,1,-6),Size=UDim2.new(1,-24,0,2),BackgroundColor3=tone,BackgroundTransparency=persistent and 1 or 0,BorderSizePixel=0,ZIndex=1103});corner(prog,2)
...
function Section:AddToggle(o)
    ...
    function c:Set(v,fire)apply(v,fire)end;function c:Get()return val end;function c:GetDefault()return def end;function c:SetDisabled(v)self.Disabled=not not v;tr.Active=not self.Disabled;tr.BackgroundTransparency=self.Disabled and .45 or 0 end;flag(c,o,def);w:_connect(tr.MouseButton1Click,function()if not c.Disabled then apply(not val,true)end end);apply(def,false);return c
end
