-- AstraUI V5.1.0 - Stable Clean Rebuild
...
function AstraUI:CreateWindow(o)
    ...
    w:Center()
    if o.KeySystem and o.KeySystem.Enabled then
        task.defer(function() w:_showKeyGate(o.KeySystem) end)
    end
    return w
end
...
function Window:_showKeyGate(o)
    o=o or {}
    if self._keyGate and self._keyGate.Parent then self._keyGate:Destroy() end

    local v=viewport()
    local mobile=self:IsMobile()
    local panelW=math.min(mobile and 320 or 390,v.X-28)
    local panelH=math.min(mobile and 220 or 250,v.Y-28)

    local back=create("Frame",{
        Parent=self.Overlay,Size=UDim2.fromScale(1,1),
        BackgroundColor3=Color3.new(0,0,0),BackgroundTransparency=.36,
        BorderSizePixel=0,ZIndex=1500,Active=true,
    })
    self._keyGate=back

    local panel=create("Frame",{
        Parent=back,AnchorPoint=Vector2.new(.5,.5),Position=UDim2.fromScale(.5,.5),
        Size=UDim2.fromOffset(panelW,panelH),BackgroundColor3=self.Theme.Surface,
        BorderSizePixel=0,ZIndex=1501,ClipsDescendants=true,
    })
    panel:SetAttribute("AstraBaseRadius",14)
    corner(panel,14)
    stroke(panel,self.Theme.Border,.10,1)

    local badge=label(panel,"ACCESS KEY",8,self.Theme.Accent,Enum.Font.GothamBold,Enum.TextXAlignment.Center)
    badge.Position=UDim2.fromOffset(18,16)
    badge.Size=UDim2.fromOffset(104,24)
    badge.BackgroundTransparency=0
    badge.BackgroundColor3=self.Theme.AccentSoft
    badge:SetAttribute("AstraBaseRadius",12)
    corner(badge,12)

    local title=label(panel,o.Title or "Astra Access",mobile and 17 or 18,self.Theme.TextPrimary,Enum.Font.GothamBold)
    title.Position=UDim2.fromOffset(18,51)
    ...
    desc.Position=UDim2.fromOffset(18,80)
    desc.Size=UDim2.new(1,-36,0,30)
    desc.TextWrapped=true
    desc.TextTruncate=Enum.TextTruncate.AtEnd
    desc.TextYAlignment=Enum.TextYAlignment.Top

    local inputY=mobile and 114 or 118
    local input=create("TextBox",{
        Parent=panel,Position=UDim2.fromOffset(18,inputY),Size=UDim2.new(1,-36,0,40),
        BackgroundColor3=self.Theme.Surface3,BorderSizePixel=0,Text="",
        PlaceholderText=o.Placeholder or "Enter key...",TextColor3=self.Theme.TextPrimary,
        PlaceholderColor3=self.Theme.TextSecondary,TextSize=11,Font=Enum.Font.Gotham,
        ClearTextOnFocus=false,ZIndex=1502,TextXAlignment=Enum.TextXAlignment.Left,
    })
    ...
    errorText.Position=UDim2.fromOffset(18,inputY+43)
    errorText.Size=UDim2.new(1,-36,0,16)
    errorText.TextTruncate=Enum.TextTruncate.AtEnd

    local unlock=button(panel,o.ButtonText or "Unlock",self.Theme,"Primary")
    unlock.Position=UDim2.new(0,18,1,-50)
    unlock.Size=UDim2.new(1,-36,0,36)
    unlock.ZIndex=1502

    local busy=false
    local function finish(valid,info,message,key)
        if valid then
            info=type(info)=="table" and info or {}
            info.Valid=true
            info.Key=key
            if not info.Status then info.Status=message or "Active" end
            self:SetKeyInfo(info)
            if back.Parent then back:Destroy() end
            self._keyGate=nil
            self:Notify({Title="Access granted",Content=info.Tier and ("Tier: "..tostring(info.Tier)) or "Key validated.",Type="success",Duration=3})
        else
            errorText.Text=tostring(message or "Invalid key")
            stroke(input,self.Theme.Danger,.05,1)
            tween(input,.08,{Position=UDim2.fromOffset(15,inputY)},Enum.EasingStyle.Quad,Enum.EasingDirection.Out)
            task.delay(.08,function()
                if input.Parent then tween(input,.10,{Position=UDim2.fromOffset(18,inputY)},Enum.EasingStyle.Quad,Enum.EasingDirection.Out) end
            end)
        end
    end
    ...
    local function submit()
        ...
        errorText.Text=""
        if key=="" then
            finish(false,nil,"Enter a key first.",key)
            return
        end

        busy=true
        local previous=unlock.Text
        ...
        if type(o.Validate)=="function" then
            local ok,res=pcall(o.Validate,key)
            if not ok then
                message="Validator error: "..tostring(res)
            elseif type(res)=="table" then
                valid=res.Valid==true or res.Success==true
                info=copy(res.KeyInfo or res.Info or {})
                message=res.Message
            else
                valid=res==true
            end
        ...
        -- Optional direct-key fallback is useful for local examples and simple hubs.
        if not valid and o.Key~=nil then
            valid=key==trimText(o.Key)
        end

        busy=false
        if unlock.Parent then
            unlock.Text=previous
            unlock.Active=true
        end
        finish(valid,info,message or (valid and "Active" or "Invalid key"),key)
    end
    ...
    task.defer(function()
        if input.Parent and not mobile then input:CaptureFocus() end
    end)
