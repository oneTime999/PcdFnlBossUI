-- AstraUI V5.3.0 - Design Recovery / Stable Interaction Core
...
function AstraUI:CreateWindow(o)
    ...
    local scale=Instance.new("UIScale");if o.Scale==nil and w.DeviceInfo and w.DeviceInfo.Platform=="Mobile" then w.Scale=.90 end;scale.Scale=w.Scale;scale.Parent=main;w.ScaleObject=scale
    ...
    local function deviceChanged()
        ...
        if old.Layout~=w.DeviceInfo.Layout or old.Platform~=w.DeviceInfo.Platform or old.PreferredInput~=w.DeviceInfo.PreferredInput then
...
function Window:_responsive()
    local mobile=self.DeviceInfo.Platform=="Mobile"
    local compact=self.CurrentSize.X<720
    local collapsed=mobile or compact
    if self._manualSidebar~=nil then collapsed=self._manualSidebar end

    self.SidebarCollapsed=collapsed
    local sw
    if collapsed then
        sw=mobile and 50 or 56
    else
        sw=self.CurrentSize.X<860 and 164 or 176
    end

    self.Sidebar.Size=UDim2.new(0,sw,1,0)
    self.Content.Position=UDim2.fromOffset(sw,0)
    self.Content.Size=UDim2.new(1,-sw,1,0)
    self.SearchHolder.Visible=not collapsed
    self.BrandTitle.Visible=not collapsed
    self.BrandSubtitle.Visible=not collapsed
    self.Footer.Visible=not collapsed
    self.ResizeGrip.Visible=self.Resizable and self:IsComputer() and not self.Minimized

    for _,tab in ipairs(self.Tabs)do
        tab:_sidebar(collapsed)
    end
...
local function safeMax(w)
    ...
    local marginX=mobile and 20 or 44
    local marginY=mobile and 24 or 44
    local scale=math.max(.01,w.Scale)
    return Vector2.new(
        math.max(280,math.floor((v.X-marginX)/scale)),
        math.max(240,math.floor((v.Y-marginY)/scale))
    )
end
...
local function preset(w,name)
    ...
    if w:IsMobile() then
        if w.DeviceInfo.Orientation=="Portrait" then
            p={
                small=Vector2.new(math.min(300,sm.X),math.min(380,sm.Y)),
                normal=Vector2.new(math.min(340,sm.X),math.min(500,sm.Y)),
                large=Vector2.new(math.min(380,sm.X),math.min(620,sm.Y)),
                max=sm,
            }
        else
            -- Landscape phones should not start almost full-screen.
            local normalW=math.min(700,math.floor(sm.X*.82))
            local normalH=math.min(350,math.floor(sm.Y*.92))
            p={
                small=Vector2.new(math.min(500,sm.X),math.min(270,sm.Y)),
                normal=Vector2.new(math.min(640,normalW),math.min(330,normalH)),
                large=Vector2.new(math.min(720,sm.X),math.min(370,sm.Y)),
                max=sm,
            }
        end
    else
        p={
            small=Vector2.new(580,370),
            normal=w.DefaultSize,
            large=Vector2.new(math.max(w.DefaultSize.X,980),math.max(w.DefaultSize.Y,620)),
            max=sm,
        }
    end

    local target=p[name] or p.normal
    local lim=w:GetWindowSizeLimits()
    return Vector2.new(
        clamp(target.X,lim.Min.X,lim.Max.X),
        clamp(target.Y,lim.Min.Y,lim.Max.Y)
    )
end
...
function Window:GetWindowSizeLimits()
    ...
    local maxv=Vector2.new(
        math.max(280,math.min(requestedMax.X,sm.X)),
        math.max(240,math.min(requestedMax.Y,sm.Y))
    )

    local minv
    if self:IsMobile() then
        if self.DeviceInfo.Orientation=="Portrait" then
            minv=Vector2.new(math.min(280,maxv.X),math.min(360,maxv.Y))
        else
            minv=Vector2.new(math.min(500,maxv.X),math.min(280,maxv.Y))
        end
    else
        minv=Vector2.new(
            math.min(math.max(480,requestedMin.X),maxv.X),
            math.min(math.max(320,requestedMin.Y),maxv.Y)
        )
    end
...
function Window:SetScale(v)
    local lo,hi=.70,1.30
    if self:IsMobile() then lo,hi=.78,1.12 end
    self.Scale=clamp(tonumber(v) or 1,lo,hi)
...
local function card(s,name,desc,h)
    ...
    local c=setmetatable({Window=w,Section=s,Instance=r,Title=a,Description=d,ActionReserve=0},Control)
...
function Section:AddQuickActions(o)
    ...
    local cols=math.max(1,math.min(mobile and 2 or 4,#acts>0 and #acts or 1))
    local rows=math.max(1,math.ceil(math.max(1,#acts)/cols))
    local rowH=38
    local gap=8
    local hostH=rows*rowH+(rows-1)*gap
    local r,c=rich(self,o.Name or "Quick Actions",o.Description or "Frequently used actions",58+hostH+14)
    local host=create("Frame",{Parent=r,Position=UDim2.fromOffset(14,54),Size=UDim2.new(1,-28,0,hostH),BackgroundTransparency=1})
    ...
    for i,a in ipairs(acts)do
        ...
        b.Active=type(a.Callback)=="function"
        if not b.Active then b.BackgroundTransparency=.45 end
        self.Window:_connect(b.Activated,function()
            if b.Active then safeCall(a.Callback) end
        end)
    end
    return c
end
