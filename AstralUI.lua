...
-- ============================================================================
-- AstraUI V4.0.1 - Quality / Layout / Typography audit hotfix
-- Focus: eliminate text collisions, overflow, clipping regressions, context-menu
-- sizing issues, and responsive layout inconsistencies across every V4 surface.
-- ============================================================================
;(function()
local V401_VERSION = "4.0.1-quality"

local function v401Viewport()
    local camera = workspace.CurrentCamera
    return camera and camera.ViewportSize or Vector2.new(1280,720)
end

local function v401Rect(gui)
    if not gui or not gui:IsA("GuiObject") then return nil end
    local p,s = gui.AbsolutePosition, gui.AbsoluteSize
    return {x1=p.X,y1=p.Y,x2=p.X+s.X,y2=p.Y+s.Y,w=s.X,h=s.Y}
end

local function v401Intersects(a,b)
    if not a or not b then return false,0 end
    local w = math.min(a.x2,b.x2)-math.max(a.x1,b.x1)
    local h = math.min(a.y2,b.y2)-math.max(a.y1,b.y1)
    if w>0 and h>0 then return true,w*h end
    return false,0
end


local function v401ActuallyVisible(gui,stopAt)
    local current=gui
...
local function v401IsText(gui)
    return gui and (gui:IsA("TextLabel") or gui:IsA("TextButton") or gui:IsA("TextBox"))
end

local function v401TextIsShort(gui)
    if not gui or not gui:IsA("GuiObject") then return false end
    local h = gui.AbsoluteSize.Y
    if h <= 0 then h = gui.Size.Y.Offset end
    return h > 0 and h <= 26
end

local function v401NormalizeText(gui)
    if not v401IsText(gui) then return end
    pcall(function() gui.TextScaled = false end)
    ...
    if gui:IsA("TextBox") then
        if not gui.MultiLine then gui.TextWrapped = false end
        return
    end

    -- Small labels are semantic one-line values/titles. Allowing them to wrap
    -- is what caused the majority of V4 visual collisions in narrow layouts.
    if v401TextIsShort(gui) and gui:GetAttribute("AstraAllowWrap") ~= true then
        gui.TextWrapped = false
        gui.TextTruncate = Enum.TextTruncate.AtEnd
    elseif gui.TextWrapped then
        -- Multiline copy should start at the top of its box; this avoids the
        -- optical overlap produced by vertically-centered wrapped paragraphs.
        gui.TextYAlignment = Enum.TextYAlignment.Top
    end
end

local function v401NormalizeTree(root)
    if not root then return end
    if v401IsText(root) then v401NormalizeText(root) end
    for _,obj in ipairs(root:GetDescendants()) do
        if v401IsText(obj) then v401NormalizeText(obj) end
        if obj:IsA("ScrollingFrame") then
            obj.ScrollingDirection = Enum.ScrollingDirection.Y
            obj.VerticalScrollBarInset = Enum.ScrollBarInset.None
            if obj.ScrollBarThickness > 0 then obj.ScrollBarThickness = math.clamp(obj.ScrollBarThickness,2,4) end
        end
    end
end

local function v401GrowWrappedLabel(label)
    if not label:IsA("TextLabel") or not label.TextWrapped or not label.Visible then return false end
    if label.Size.Y.Scale ~= 0 or label.AnchorPoint.Y > 0.01 then return false end
    local actual = label.AbsoluteSize.Y
    local needed = label.TextBounds.Y
    if needed > actual + 2 then
        local delta = math.ceil(needed-actual)+2
        label.Size = UDim2.new(label.Size.X.Scale,label.Size.X.Offset,0,label.Size.Y.Offset+delta)
        return true
    end
    return false
end

local function v401GrowRow(row)
    if not row or not row:IsA("GuiObject") or row.Size.Y.Scale ~= 0 then return false end
    if row:GetAttribute("AstraV38Role") ~= "Row" and row:GetAttribute("AstraV4AutoRepair") ~= true then return false end
    local rowTop = row.AbsolutePosition.Y
    local current = row.AbsoluteSize.Y
    if current <= 0 then current = row.Size.Y.Offset end
    local maxBottom = current
    for _,child in ipairs(row:GetChildren()) do
        if child:IsA("GuiObject") and child.Visible and child.AnchorPoint.Y < 0.5 and child.Position.Y.Scale < 0.75 then
            local bottom = (child.AbsolutePosition.Y-rowTop)+child.AbsoluteSize.Y
            if bottom > maxBottom then maxBottom = bottom end
        end
    end
    if maxBottom > current + 2 then
        local newHeight = math.ceil(maxBottom+8)
        row.Size = UDim2.new(row.Size.X.Scale,row.Size.X.Offset,0,newHeight)
        return true
    end
    return false
end

local function v401RepairPagePadding(page)
    if not page or not page:IsA("ScrollingFrame") then return end
    local pad = page:FindFirstChildOfClass("UIPadding")
    if pad then
        local current = pad.PaddingBottom.Offset
        if current < 18 then pad.PaddingBottom = UDim.new(0,18) end
    end
end

local function v401RepairPass(window,root)
    if not window or window.Destroyed then return end
    root = root or window.ScreenGui
    if not root then return end
    v401NormalizeTree(root)

    -- Pass 1: let multiline copy consume the height it actually needs.
    for _,obj in ipairs(root:GetDescendants()) do
        if obj:IsA("TextLabel") then v401GrowWrappedLabel(obj) end
    end

    -- Pass 2: grow rows after child text has stabilized.
    for _,obj in ipairs(root:GetDescendants()) do
        if obj:IsA("GuiObject") and obj:GetAttribute("AstraV38Role") == "Row" then v401GrowRow(obj) end
        if obj:IsA("ScrollingFrame") then v401RepairPagePadding(obj) end
    end
end

function Window:RepairLayout(root)
    if self.Destroyed then return false end
    root = root or self.ScreenGui
    v401RepairPass(self,root)
    task.defer(function()
        if self.Destroyed then return end
        v401RepairPass(self,root)
        task.delay(0.08,function()
            if not self.Destroyed then v401RepairPass(self,root) end
        end)
    end)
    return true
end

function Window:AuditLayout(root)
    root = root or self.ScreenGui
    local report = {
        Version = V401_VERSION,
        TextObjects = 0,
        IntentionalTruncations = 0,
        VerticalTextClipping = 0,
        RowOverflows = 0,
        SuspiciousTextOverlaps = 0,
        ViewportOverflows = 0,
        Issues = {},
    }
    if not root then report.OK=false; return report end

    local vp = v401Viewport()
    local textByParent = {}

    for _,obj in ipairs(root:GetDescendants()) do
        if v401IsText(obj) and v401ActuallyVisible(obj,root) then
            report.TextObjects += 1
            if obj:IsA("TextLabel") or obj:IsA("TextButton") then
                local bounds = obj.TextBounds
                local size = obj.AbsoluteSize
                if bounds.Y > size.Y + 2 then
                    report.VerticalTextClipping += 1
                    table.insert(report.Issues,{Type="VerticalTextClipping",Object=obj:GetFullName(),Needed=math.ceil(bounds.Y),Available=math.floor(size.Y)})
                elseif bounds.X > size.X + 2 then
                    report.IntentionalTruncations += 1
                end
            end
            local parent = obj.Parent
            if parent and parent:IsA("GuiObject") then
                textByParent[parent] = textByParent[parent] or {}
                table.insert(textByParent[parent],obj)
            end
        end

        if obj:IsA("GuiObject") and v401ActuallyVisible(obj,root) and obj:GetAttribute("AstraV38Role") == "Row" then
            local r = v401Rect(obj)
            if r then
                for _,child in ipairs(obj:GetChildren()) do
                    if child:IsA("GuiObject") and child.Visible then
                        local c = v401Rect(child)
                        if c and c.y2 > r.y2 + 3 and child.AnchorPoint.Y < 0.5 then
                            report.RowOverflows += 1
                            table.insert(report.Issues,{Type="RowOverflow",Object=obj:GetFullName(),Child=child.Name,Pixels=math.ceil(c.y2-r.y2)})
                            break
                        end
                    end
                end
        ...
        -- Only inspect regular application surfaces. Popovers, notifications,
        -- drawers and modals intentionally live outside the root rectangle.
        if obj:IsA("GuiObject") and v401ActuallyVisible(obj,root) and obj.ZIndex < 500 and obj.AbsoluteSize.X > 8 and obj.AbsoluteSize.Y > 8 then
            local p,s = obj.AbsolutePosition,obj.AbsoluteSize
            if p.X+s.X < -12 or p.Y+s.Y < -12 or p.X > vp.X+12 or p.Y > vp.Y+12 then
                report.ViewportOverflows += 1
            end
        end
    end

    -- Text-vs-text overlap audit. Ignore labels nested inside different controls
    -- and tiny intersections (baseline/anti-aliasing can create 1px touches).
    for parent,items in pairs(textByParent) do
        if #items <= 12 and not parent:IsA("TextButton") then
            for i=1,#items do
                for j=i+1,#items do
                    local a,b=items[i],items[j]
                    if a.ZIndex==b.ZIndex and a.Visible and b.Visible then
                        local hit,area=v401Intersects(v401Rect(a),v401Rect(b))
                        if hit and area>24 then
                            local sameBand = math.abs(a.AbsolutePosition.X-b.AbsolutePosition.X) < math.max(a.AbsoluteSize.X,b.AbsoluteSize.X)*0.75
                            if sameBand then
                                report.SuspiciousTextOverlaps += 1
                                table.insert(report.Issues,{Type="TextOverlap",Parent=parent:GetFullName(),A=a.Name,B=b.Name,Area=math.floor(area)})
                            end
                        end
                    end
                end
    ...
    report.OK = report.VerticalTextClipping == 0 and report.RowOverflows == 0 and report.SuspiciousTextOverlaps == 0
    return report
end

local _V401QualityReport = Window.GetQualityReport
function Window:GetQualityReport()
    local base = type(_V401QualityReport)=="function" and _V401QualityReport(self) or {}
    local layout = self:AuditLayout()
    base.Version = V401_VERSION
    base.Layout = layout
    base.LayoutOK = layout.OK
    return base
end

-- --------------------------------------------------------------------------
-- Compact, viewport-aware context menu. Long press no longer opens a menu if
-- the finger has already moved into a scroll gesture.
-- --------------------------------------------------------------------------
function Window:ContextMenu(anchor,items,options)
    options=options or {}; items=items or {}
    if self._v4Context and self._v4Context.Parent then self._v4Context:Destroy() end

    local overlay=Instance.new("TextButton")
    overlay.Name="AstraContextOverlay"; overlay.Text=""; overlay.AutoButtonColor=false; overlay.BackgroundTransparency=1
    overlay.Size=UDim2.fromScale(1,1); overlay.ZIndex=600; overlay.Parent=self.ScreenGui

    local viewport=v401Viewport()
    local touch = tostring(self.GetPreferredInput and self:GetPreferredInput() or "") == "Touch" or UserInputService.TouchEnabled
    local itemHeight = touch and 40 or 34
    local gap = 2
    local width = math.clamp(tonumber(options.Width) or 194,154,math.max(154,viewport.X-16))
    local desiredHeight = 8 + #items*itemHeight + math.max(0,#items-1)*gap
    local maxHeight = math.max(itemHeight+8,viewport.Y-24)
    local height = math.min(desiredHeight,maxHeight)

    local menu=Instance.new("ScrollingFrame")
    menu.Name="AstraContextMenu"; menu.BackgroundColor3=self.Theme.Surface3; menu.BorderSizePixel=0
    menu.Size=UDim2.fromOffset(width,height); menu.CanvasSize=UDim2.new(); menu.AutomaticCanvasSize=Enum.AutomaticSize.Y
    menu.ScrollBarThickness=desiredHeight>maxHeight and 2 or 0; menu.ScrollingDirection=Enum.ScrollingDirection.Y
    menu.ZIndex=601; menu.Parent=overlay
    local corner=Instance.new("UICorner"); corner.CornerRadius=UDim.new(0,12); corner.Parent=menu
    local stroke=Instance.new("UIStroke"); stroke.Color=self.Theme.BorderDefault or self.Theme.Border; stroke.Transparency=0.35; stroke.Thickness=1; stroke.Parent=menu
    local pad=Instance.new("UIPadding"); pad.PaddingLeft=UDim.new(0,4); pad.PaddingRight=UDim.new(0,4); pad.PaddingTop=UDim.new(0,4); pad.PaddingBottom=UDim.new(0,4); pad.Parent=menu
    local layout=Instance.new("UIListLayout"); layout.Padding=UDim.new(0,gap); layout.SortOrder=Enum.SortOrder.LayoutOrder; layout.Parent=menu

    local pos = anchor and anchor.AbsolutePosition or UserInputService:GetMouseLocation()
    local anchorSize = anchor and anchor.AbsoluteSize or Vector2.zero
    local belowY = pos.Y + anchorSize.Y + 5
    local aboveY = pos.Y - height - 5
    local y = (belowY+height <= viewport.Y-8) and belowY or math.max(8,aboveY)
    local x = math.clamp(pos.X,8,math.max(8,viewport.X-width-8))
    menu.Position=UDim2.fromOffset(x,y)

    self._v4Context=overlay
    for i,item in ipairs(items) do
        local b=Instance.new("TextButton")
        b.AutoButtonColor=false; b.BackgroundColor3=self.Theme.Surface3; b.BorderSizePixel=0
        b.Text=tostring(item.Text or item.Name or "Action"); b.TextSize=10; b.Font=Enum.Font.GothamMedium
        b.TextColor3=item.Danger and self.Theme.Danger or self.Theme.TextPrimary
        b.TextXAlignment=Enum.TextXAlignment.Left; b.TextWrapped=false; b.TextTruncate=Enum.TextTruncate.AtEnd
        b.Size=UDim2.new(1,0,0,itemHeight); b.LayoutOrder=i; b.ZIndex=602; b.Parent=menu
        local bc=Instance.new("UICorner"); bc.CornerRadius=UDim.new(0,9); bc.Parent=b
        local bp=Instance.new("UIPadding"); bp.PaddingLeft=UDim.new(0,12); bp.PaddingRight=UDim.new(0,10); bp.Parent=b
...
function Window:AttachContextMenu(gui,items)
    ...
    self:_connect(gui.InputBegan,function(io)
        if io.UserInputType==Enum.UserInputType.MouseButton2 then
            self:ContextMenu(gui,items)
        elseif io.UserInputType==Enum.UserInputType.Touch then
            touchToken += 1
            local token=touchToken
            local start=io.Position
            task.delay(0.55,function()
                if token~=touchToken or not gui.Parent then return end
                local moved=(io.Position-start).Magnitude
                if moved<=12 and io.UserInputState~=Enum.UserInputState.End then self:ContextMenu(gui,items) end
            end)
        end
    end)
    self:_connect(gui.InputEnded,function(io)
        if io.UserInputType==Enum.UserInputType.Touch then touchToken += 1 end
    end)
end

-- --------------------------------------------------------------------------
-- Quick actions automatically stack when the row becomes too narrow. This is
-- one of the main portrait/mobile overflow sources in the V4 dashboard.
-- --------------------------------------------------------------------------
local _V401QuickActions = Tab._addQuickActions
function Tab:_addQuickActions(parent,data)
    local object=_V401QuickActions(self,parent,data)
    local row=object and object.Instance
    if not row then return object end
    local holder
    for _,child in ipairs(row:GetChildren()) do
        if child:IsA("Frame") and child:FindFirstChildOfClass("UIListLayout") then holder=child end
    end
    if not holder then return object end
    local layout=holder:FindFirstChildOfClass("UIListLayout")
    local buttons=object.Buttons or {}

    local function apply()
        if not row.Parent or self.Window.Destroyed then return end
        local available=math.max(1,row.AbsoluteSize.X-24)
        local total=0
        for _,b in ipairs(buttons) do total += math.max(72,b.AbsoluteSize.X>0 and b.AbsoluteSize.X or b.Size.X.Offset) end
        total += math.max(0,#buttons-1)*6
        local stack = total > available or available < 360
        if stack then
            layout.FillDirection=Enum.FillDirection.Vertical
            layout.HorizontalAlignment=Enum.HorizontalAlignment.Left
            layout.VerticalAlignment=Enum.VerticalAlignment.Top
            holder.Position=UDim2.fromOffset(12,43)
            holder.Size=UDim2.new(1,-24,0,#buttons*34+math.max(0,#buttons-1)*6)
            for _,b in ipairs(buttons) do b.Size=UDim2.new(1,0,0,34) end
            row.Size=UDim2.new(row.Size.X.Scale,row.Size.X.Offset,0,52+#buttons*34+math.max(0,#buttons-1)*6)
        else
            layout.FillDirection=Enum.FillDirection.Horizontal
            layout.HorizontalAlignment=Enum.HorizontalAlignment.Right
            layout.VerticalAlignment=Enum.VerticalAlignment.Center
            holder.Position=UDim2.fromOffset(12,42)
            holder.Size=UDim2.new(1,-24,0,28)
            for _,b in ipairs(buttons) do
                local text=tostring(b.Text or "")
                b.Size=UDim2.fromOffset(math.clamp(46+#text*5,72,126),28)
            end
            row.Size=UDim2.new(row.Size.X.Scale,row.Size.X.Offset,0,78)
        end
    end

    task.defer(apply)
    if workspace.CurrentCamera then self.Window:_connect(workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"),function() task.defer(apply) end) end
    object.RefreshLayout=apply
    return object
end

-- --------------------------------------------------------------------------
-- Lists, grids and one-line metadata values must never wrap over adjacent
-- labels. Apply an explicit ellipsis policy to the high-density components.
-- --------------------------------------------------------------------------
local _V401List = Tab._addList
function Tab:_addList(parent,data)
    local object=_V401List(self,parent,data)
    task.defer(function()
        if not object or not object.Instance then return end
        for _,obj in ipairs(object.Instance:GetDescendants()) do
            if obj:IsA("TextLabel") and obj.Size.Y.Offset<=20 then obj.TextWrapped=false; obj.TextTruncate=Enum.TextTruncate.AtEnd end
        end
    end)
    return object
end

local _V401Grid = Tab._addDataGrid
function Tab:_addDataGrid(parent,data)
    local object=_V401Grid(self,parent,data)
    task.defer(function()
        if not object or not object.Instance then return end
        for _,obj in ipairs(object.Instance:GetDescendants()) do
            if obj:IsA("TextLabel") or obj:IsA("TextButton") then obj.TextWrapped=false; obj.TextTruncate=Enum.TextTruncate.AtEnd end
        end
    end)
    return object
end
...
-- --------------------------------------------------------------------------
-- Generic automatic repair hooks.
-- --------------------------------------------------------------------------
local _V401SelectTab = Window.SelectTab
function Window:SelectTab(tab)
    local result=_V401SelectTab(self,tab)
    task.defer(function() if not self.Destroyed then self:RepairLayout(tab and tab.Page or self.ScreenGui) end end)
    return result
end

local _V401SetTheme = Window.SetTheme
function Window:SetTheme(themePatch)
    local result=_V401SetTheme(self,themePatch)
    task.defer(function() if not self.Destroyed then self:RepairLayout() end end)
    return result
end

local _V401Density = Window.SetDensity
if type(_V401Density)=="function" then
    function Window:SetDensity(mode)
        local result=_V401Density(self,mode)
        task.defer(function() if not self.Destroyed then self:RepairLayout() end end)
        return result
    end
end

local _V401Scale = Window.SetScale
if type(_V401Scale)=="function" then
    function Window:SetScale(value)
        local result=_V401Scale(self,value)
        task.defer(function() if not self.Destroyed then self:RepairLayout() end end)
        return result
    end
end

local _V401Sidebar = Window.SetSidebarCollapsed
if type(_V401Sidebar)=="function" then
    function Window:SetSidebarCollapsed(state,instant)
        local result=_V401Sidebar(self,state,instant)
        task.delay(instant and 0 or 0.22,function() if not self.Destroyed then self:RepairLayout() end end)
        return result
    end
end

local _V401CreateWindow = AstraUI.CreateWindow
function AstraUI:CreateWindow(options)
    local window=_V401CreateWindow(self,options)
    if not window then return window end
    window._v401QualityMode=true

    local function normalizeAdded(obj)
        if v401IsText(obj) then task.defer(function() if obj.Parent then v401NormalizeText(obj) end end) end
    end
    if window.ScreenGui then
        window:_connect(window.ScreenGui.DescendantAdded,normalizeAdded)
        v401NormalizeTree(window.ScreenGui)
    end
    if workspace.CurrentCamera then
        window:_connect(workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"),function()
            task.defer(function() if not window.Destroyed then window:RepairLayout() end end)
        end)
    end
    task.defer(function()
        if window.Destroyed then return end
        window:RepairLayout()
        task.delay(0.25,function() if not window.Destroyed then window:RepairLayout() end end)
    end)
    return window
end

AstraUI.Version = V401_VERSION
end)()


-- ============================================================================
-- AstraUI V4.0.2 - Interaction & Spacing pass
-- Focus: real visual breathing room, reliable button affordance, touch-safe
-- action spacing, and removal of "looks clickable but does nothing" states.
-- ============================================================================
;(function()
local V402_VERSION = "4.0.2-interaction"

local function v402ListLayout(parent)
    return parent and parent:FindFirstChildOfClass("UIListLayout") or nil
end

local function v402SetPadding(layout, pixels)
    if layout then layout.Padding = UDim.new(0, pixels) end
end

local function v402DirectTextBottom(row)
    local bottom = 0
    for _,child in ipairs(row:GetChildren()) do
        if child:IsA("TextLabel") and child.Visible and child.Position.Y.Scale == 0 and child.Position.Y.Offset <= 36 then
            local h = child.Size.Y.Offset
            if h <= 0 then h = child.AbsoluteSize.Y end
            bottom = math.max(bottom, child.Position.Y.Offset + math.max(0,h))
        end
    end
    return bottom
end

-- Give the content underneath a title/description a real 7-9px breathing room.
-- Older rich controls often started their body at y=40-43 while the description
-- itself ended at y=42-43, which made the UI technically non-overlapping but
-- visually cramped.
local function v402RepairRowSpacing(row)
    if not row or not row:IsA("Frame") or not row.Parent then return end
    local textBottom = v402DirectTextBottom(row)
    if textBottom <= 0 then return end
    local target = textBottom + 8
    local maxDelta = 0

    for _,child in ipairs(row:GetChildren()) do
        if child:IsA("GuiObject") and not child:IsA("TextLabel") then
            local p = child.Position
            local centeredRightControl = p.Y.Scale ~= 0 or child.AnchorPoint.Y >= 0.45
            local bodyCandidate = not centeredRightControl and p.Y.Scale == 0 and p.Y.Offset >= 34 and p.Y.Offset <= 70
            if bodyCandidate and p.Y.Offset < target then
                local delta = target - p.Y.Offset
    ...
    if maxDelta > 0 and row.AutomaticSize == Enum.AutomaticSize.None and row.Size.Y.Scale == 0 then
        row.Size = UDim2.new(row.Size.X.Scale,row.Size.X.Offset,0,row.Size.Y.Offset + maxDelta)
    end
end

local function v402RepairTree(root)
    if not root then return end
    if root:IsA("Frame") then v402RepairRowSpacing(root) end
    for _,obj in ipairs(root:GetDescendants()) do
        if obj:IsA("Frame") then v402RepairRowSpacing(obj) end
    end
end

-- Stronger spacing contract for sections. Sections and their controls must
-- never visually merge into a continuous slab even when the content is dense.
local _V402CreateSection = Tab.CreateSection
function Tab:CreateSection(options)
    local section = _V402CreateSection(self,options)
    if not section then return section end

    local content = section.Content or section.Frame
    v402SetPadding(v402ListLayout(content),10)
    if section.Frame and section.Frame ~= content then
        v402SetPadding(v402ListLayout(section.Frame),10)
    end
    if self.Page then
        v402SetPadding(v402ListLayout(self.Page),12)
    end

    task.defer(function()
        if section.Frame and section.Frame.Parent and not self.Window.Destroyed then
            v402RepairTree(section.Frame)
        end
    end)
    return section
end

-- Direct tab-level components should receive the same separation as sections.
local _V402CreateTab = Window.CreateTab
function Window:CreateTab(options)
    local tab = _V402CreateTab(self,options)
    if tab and tab.Page then
        v402SetPadding(v402ListLayout(tab.Page),12)
    end
    return tab
end

-- Quick Actions: more space between the description and buttons, larger gaps
-- between adjacent actions, and honest disabled styling when no callback exists.
local _V402QuickActions = Tab._addQuickActions
function Tab:_addQuickActions(parent,data)
    data = data or {}
    local object = _V402QuickActions(self,parent,data)
    if not object or not object.Instance then return object end

    local row = object.Instance
    local holder,layout
    for _,child in ipairs(row:GetChildren()) do
        if child:IsA("Frame") then
            local candidate = child:FindFirstChildOfClass("UIListLayout")
            if candidate then holder=child; layout=candidate; break end
        end
    end
    if not holder or not layout then return object end

    local actions = data.Actions or {}
    local buttons = object.Buttons or {}
    local gap = math.max(8,tonumber(data.Gap) or 10)
    ...
    local function apply()
        ...
        for _,b in ipairs(buttons) do
            ...
            if w <= 0 then w = math.max(78,b.AbsoluteSize.X) end
            total += math.max(78,w)
        end
        total += math.max(0,#buttons-1)*gap

        local stack = total > available or available < 390
        if stack then
            layout.FillDirection = Enum.FillDirection.Vertical
            layout.HorizontalAlignment = Enum.HorizontalAlignment.Left
            layout.VerticalAlignment = Enum.VerticalAlignment.Top
            holder.Position = UDim2.fromOffset(12,51)
            holder.Size = UDim2.new(1,-24,0,#buttons*36 + math.max(0,#buttons-1)*gap)
            for _,b in ipairs(buttons) do b.Size=UDim2.new(1,0,0,36) end
            row.Size = UDim2.new(row.Size.X.Scale,row.Size.X.Offset,0,61 + #buttons*36 + math.max(0,#buttons-1)*gap)
        else
            layout.FillDirection = Enum.FillDirection.Horizontal
            layout.HorizontalAlignment = Enum.HorizontalAlignment.Right
            layout.VerticalAlignment = Enum.VerticalAlignment.Center
            ...
            for _,b in ipairs(buttons) do
                ...
                b.Size = UDim2.fromOffset(math.clamp(54+#label*5,78,138),32)
            end
            row.Size = UDim2.new(row.Size.X.Scale,row.Size.X.Offset,0,91)
        end
    end
    ...
    task.defer(apply)
    if workspace.CurrentCamera then
        self.Window:_connect(workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"),function() task.defer(apply) end)
    end
    return object
end

-- A button with no callback should not masquerade as a working action.
-- We keep the control visible for layouts/playgrounds, but render it disabled.
local _V402Button = Tab._addButton
function Tab:_addButton(parent,data)
    data = data or {}
    local object = _V402Button(self,parent,data)
    if object and object.Instance and type(data.Callback) ~= "function" then
        local action
        for _,child in ipairs(object.Instance:GetChildren()) do
            if child:IsA("TextButton") then action=child end
        end
        if action then
            action.Active=false
            action.Selectable=false
            action.TextTransparency=0.42
            action.BackgroundTransparency=0.28
        end
    end
    return object
end

-- Context-menu items follow the same rule: actions with no callback are
-- visibly disabled rather than silently accepting a tap.
local _V402ContextMenu = Window.ContextMenu
function Window:ContextMenu(anchor,items,options)
    items = items or {}
    local overlay = _V402ContextMenu(self,anchor,items,options)
    if overlay then
        task.defer(function()
            if not overlay.Parent then return end
            local menu = overlay:FindFirstChild("AstraContextMenu",true)
            if not menu then return end
            local buttons={}
            for _,obj in ipairs(menu:GetChildren()) do if obj:IsA("TextButton") then table.insert(buttons,obj) end end
            table.sort(buttons,function(a,b) return a.LayoutOrder < b.LayoutOrder end)
            for i,b in ipairs(buttons) do
                local item=items[i]
                if item and type(item.Callback)~="function" then
                    b.Active=false; b.Selectable=false; b.TextTransparency=0.48; b.BackgroundTransparency=0.15
                end
            end
        end)
    end
...
-- Repair after controls are created and after viewport changes. This is not an
-- "audit" feature; it simply enforces the layout contract in normal use.
local _V402CreateWindow = AstraUI.CreateWindow
function AstraUI:CreateWindow(options)
    local window = _V402CreateWindow(self,options)
    if not window then return window end

    function window:RefreshSpacing(root)
        root = root or self.ScreenGui
        if root then v402RepairTree(root) end
        for _,tab in ipairs(self.Tabs or {}) do
            if tab.Page then v402SetPadding(v402ListLayout(tab.Page),12) end
        end
    end

    if window.ScreenGui then
        window:_connect(window.ScreenGui.DescendantAdded,function(obj)
            if obj:IsA("GuiObject") then
                task.defer(function()
                    if not window.Destroyed and obj.Parent then
                        local row = obj:IsA("Frame") and obj or obj:FindFirstAncestorOfClass("Frame")
                        if row then v402RepairRowSpacing(row) end
                    end
                end)
            end
        end)
    ...
    task.defer(function()
        if not window.Destroyed then window:RefreshSpacing() end
    end)
    task.delay(0.3,function()
        if not window.Destroyed then window:RefreshSpacing() end
    end)

    AstraUI.Version = V402_VERSION
    return window
end
...
-- ============================================================================
-- AstraUI V4.0.3 - Motion, Device & Geometry System
-- Focus: visible notification motion, reliable device classification,
-- runtime-adaptive layout metadata, configurable rounding and real window resize.
-- ============================================================================
;(function()
local V403_VERSION = "4.0.3-motion-device"
...
local function preferredInput()
    local ok, value = pcall(function() return UIS.PreferredInput end)
    if ok and value then return enumName(value) end
    if UIS.TouchEnabled and not UIS.KeyboardEnabled then return "Touch" end
    if UIS.GamepadEnabled and not UIS.KeyboardEnabled then return "Gamepad" end
    return "KeyboardAndMouse"
end

local function classifyDevice()
    local vp = viewportSize()
    local preferred = preferredInput()
    local touch = UIS.TouchEnabled == true
    local keyboard = UIS.KeyboardEnabled == true
    local mouse = UIS.MouseEnabled == true
    local gamepad = UIS.GamepadEnabled == true
    local shortest = math.min(vp.X,vp.Y)
    local longest = math.max(vp.X,vp.Y)

    local platform
    if preferred == "Touch" or (touch and not keyboard and not mouse) then
        platform = "Mobile"
    elseif preferred == "Gamepad" and not keyboard and not mouse and not touch then
        platform = "Console"
    else
        platform = "Computer"
    end
    ...
    if platform == "Mobile" then
        formFactor = (shortest >= 600 or longest >= 1180) and "Tablet" or "Phone"
    ...
    local orientation = vp.Y > vp.X and "Portrait" or "Landscape"
    local layout
    if platform == "Mobile" then
        layout = formFactor == "Tablet" and (orientation == "Portrait" and "TabletPortrait" or "TabletLandscape")
            or (orientation == "Portrait" and "PhonePortrait" or "PhoneLandscape")
    elseif platform == "Console" then
        layout = "Gamepad"
    else
        layout = vp.X < 900 and "CompactDesktop" or "Desktop"
    end

    return {
        Platform = platform,
        Type = platform, -- backwards-friendly alias
        ...
        IsComputer = platform == "Computer",
        IsConsole = platform == "Console",
    }
end

local function sameDevice(a,b)
    return a and b and a.Platform == b.Platform and a.FormFactor == b.FormFactor
        and a.Orientation == b.Orientation and a.Layout == b.Layout
        and a.PreferredInput == b.PreferredInput and a.Width == b.Width and a.Height == b.Height
end
...
local function ensureScale(gui,name)
    local found = gui:FindFirstChild(name)
    if found and found:IsA("UIScale") then return found end
    local scale = Instance.new("UIScale")
    scale.Name = name
    scale.Scale = 1
    scale.Parent = gui
...
local function collectTransparency(gui)
    local state = {}
    for _,obj in ipairs(gui:GetDescendants()) do
        if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
            state[obj] = {TextTransparency=obj.TextTransparency, BackgroundTransparency=obj.BackgroundTransparency}
        elseif obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
            state[obj] = {ImageTransparency=obj.ImageTransparency, BackgroundTransparency=obj.BackgroundTransparency}
        elseif obj:IsA("UIStroke") then
            state[obj] = {Transparency=obj.Transparency}
        elseif obj:IsA("Frame") then
            state[obj] = {BackgroundTransparency=obj.BackgroundTransparency}
        end
    end
    return state
end
...
local function applyHidden(state)
    for obj,props in pairs(state) do
        if obj and obj.Parent then
            pcall(function()
                if props.TextTransparency ~= nil then obj.TextTransparency = 1 end
                if props.ImageTransparency ~= nil then obj.ImageTransparency = 1 end
                if props.Transparency ~= nil then obj.Transparency = 1 end
                if props.BackgroundTransparency ~= nil and obj.Name ~= "AstraNotificationAccent" then
...
local function tweenTransparency(state,duration,hide)
    for obj,props in pairs(state) do
        if obj and obj.Parent then
            local target = {}
            if props.TextTransparency ~= nil then target.TextTransparency = hide and 1 or props.TextTransparency end
            if props.ImageTransparency ~= nil then target.ImageTransparency = hide and 1 or props.ImageTransparency end
            if props.Transparency ~= nil then target.Transparency = hide and 1 or props.Transparency end
            if props.BackgroundTransparency ~= nil and obj.Name ~= "AstraNotificationAccent" then
                target.BackgroundTransparency = hide and 1 or props.BackgroundTransparency
            end
            if next(target) then pcall(function() tween(obj,duration,target,Enum.EasingStyle.Quart,Enum.EasingDirection.Out) end) end
        end
    end
end

-- Full notification implementation: entrance and exit are intentionally visible
-- on touch as well as mouse/keyboard. It does not depend on hover.
function Window:Notify(options)
    options = copyTable(options or {})
    if self.Destroyed then return nil end
    self._activeNotifications = self._activeNotifications or {}

    -- Preserve V4 notification-center history.
    if self._v4Notifications then
        self._v4NotificationId = (self._v4NotificationId or 0) + 1
        local key = tostring(options.Title or "Notification") .. "\0" .. tostring(options.Content or options.Text or "")
        local found
        for _,entry in ipairs(self._v4Notifications) do if entry.Key == key then found = entry break end end
        if found then
            found.Count = (found.Count or 1) + 1
            found.Time = os.time()
        else
            table.insert(self._v4Notifications,1,{
                Id=self._v4NotificationId,Key=key,Title=options.Title or "Notification",
                Content=options.Content or options.Text or "",Type=options.Type or "info",Time=os.time(),Count=1,
            })
        end
        while #self._v4Notifications > 100 do table.remove(self._v4Notifications) end
    end
    ...
    local device = self.GetDeviceInfo and self:GetDeviceInfo() or classifyDevice()
    local width = device.IsMobile and math.clamp(device.Width - 24,240,324) or math.clamp(tonumber(options.Width) or 330,260,380)
    if self.NotificationHost then self.NotificationHost.Size = UDim2.fromOffset(width,0) end

    local card = create("Frame",{
        Parent=self.NotificationHost,Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,
        BackgroundColor3=theme.Surface3 or theme.Surface,BackgroundTransparency=0,BorderSizePixel=0,ZIndex=101,
    },{
        corner(math.max(12,math.floor(13*(self._v403Roundness or 1)))),
        stroke(theme.BorderDefault or theme.Border,0.30,1),
        padding(13,13,11,11),
    })
    card.Name = "AstraNotification"

    local accentBar = create("Frame",{
        Name="AstraNotificationAccent",Parent=card,AnchorPoint=Vector2.new(0,0.5),Position=UDim2.new(0,-13,0.5,0),
        Size=UDim2.new(0,3,0,0),BackgroundColor3=accent,BorderSizePixel=0,ZIndex=104,
    },{corner(999)})

    local layout = listLayout(card,6)
    layout.HorizontalAlignment = Enum.HorizontalAlignment.Left
    local top = create("Frame",{Parent=card,LayoutOrder=1,Size=UDim2.new(1,0,0,21),BackgroundTransparency=1,ZIndex=102})
    create("Frame",{Parent=top,Position=UDim2.fromOffset(0,6),Size=UDim2.fromOffset(8,8),BackgroundColor3=accent,BorderSizePixel=0,ZIndex=103},{corner(999)})
    local title = create("TextLabel",{
        Parent=top,BackgroundTransparency=1,Position=UDim2.fromOffset(16,0),Size=UDim2.new(1,-44,1,0),
        Text=tostring(options.Title or "Notification"),TextColor3=theme.TextPrimary or theme.Text,
        TextXAlignment=Enum.TextXAlignment.Left,TextSize=12,Font=Enum.Font.GothamBold,TextWrapped=false,
        TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=103,
    })
    local close = create("TextButton",{
        Parent=top,AutoButtonColor=false,AnchorPoint=Vector2.new(1,0),Position=UDim2.new(1,0,0,0),Size=UDim2.fromOffset(22,21),
        BackgroundTransparency=1,Text="×",TextColor3=theme.TextSecondary or theme.Muted,TextSize=17,Font=Enum.Font.Gotham,ZIndex=103,
    })
    local body = textLabel(card,options.Content or options.Text or "",11,theme.TextSecondary or theme.Muted,false)
    body.LayoutOrder=2; body.ZIndex=102; body.TextWrapped=true

    local actionHolder
    if type(options.Actions)=="table" and #options.Actions>0 then
        actionHolder=create("Frame",{Parent=card,LayoutOrder=3,Size=UDim2.new(1,0,0,32),BackgroundTransparency=1,ZIndex=102})
        create("UIListLayout",{Parent=actionHolder,FillDirection=Enum.FillDirection.Horizontal,HorizontalAlignment=Enum.HorizontalAlignment.Right,
            VerticalAlignment=Enum.VerticalAlignment.Center,SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,7)})
    end

    local progress=create("Frame",{Parent=card,LayoutOrder=4,Size=UDim2.new(1,0,0,2),BackgroundColor3=theme.Surface2,BorderSizePixel=0,ZIndex=102},{corner(999)})
    local fill=create("Frame",{Parent=progress,Size=UDim2.fromScale(1,1),BackgroundColor3=accent,BorderSizePixel=0,ZIndex=103},{corner(999)})

    local scale=ensureScale(card,"AstraNotificationMotion")
    local visualState=collectTransparency(card)
    local closed=false
    local handle={Card=card}

    local function removeFromActive()
        for i=#self._activeNotifications,1,-1 do
            if self._activeNotifications[i]==handle then table.remove(self._activeNotifications,i) break end
        end
    end
    ...
    local function closeNotification()
        ...
        tween(scale,0.18,{Scale=0.92},Enum.EasingStyle.Quart,Enum.EasingDirection.In)
        tween(accentBar,0.14,{Size=UDim2.new(0,3,0,0)},Enum.EasingStyle.Quart,Enum.EasingDirection.In)
        tween(card,0.18,{BackgroundTransparency=1},Enum.EasingStyle.Quart,Enum.EasingDirection.In)
        tweenTransparency(visualState,0.16,true)
        task.delay(0.19,finishDestroy)
    end
    ...
    if actionHolder then
        for _,action in ipairs(options.Actions) do
            local actionButton=button(actionHolder,action.Text or "Action",UDim2.fromOffset(action.Width or 80,29),action.Primary and accent or (theme.SurfaceInteractive or theme.Surface2))
            actionButton.TextSize=10; actionButton.ZIndex=103
            self:_connect(actionButton.MouseButton1Click,function()
                safeCall(action.Callback,handle)
                if action.Close~=false then closeNotification() end
            end)
        end
        -- Action buttons are created after the card skeleton, so refresh the
        -- transparency snapshot to include them in entrance/exit motion.
        visualState=collectTransparency(card)
    end
    ...
    -- Entrance: readable on mobile, not just hover-based.
    scale.Scale=0.90
    card.BackgroundTransparency=1
    applyHidden(visualState)
    accentBar.Size=UDim2.new(0,3,0,0)
    task.defer(function()
        if closed or not card.Parent then return end
        tween(scale,0.25,{Scale=1},Enum.EasingStyle.Back,Enum.EasingDirection.Out)
        tween(card,0.18,{BackgroundTransparency=0},Enum.EasingStyle.Quart,Enum.EasingDirection.Out)
        tweenTransparency(visualState,0.18,false)
        tween(accentBar,0.24,{Size=UDim2.new(0,3,1,-8)},Enum.EasingStyle.Quart,Enum.EasingDirection.Out)
    end)

    local duration=tonumber(options.Duration)
    if duration==nil then duration=4 end
    if options.Persistent==true then duration=0 end
    if duration>0 then
        tween(fill,duration,{Size=UDim2.fromScale(0,1)},Enum.EasingStyle.Linear,Enum.EasingDirection.Out)
        task.delay(duration,closeNotification)
    else
...
-- Device/runtime classification ------------------------------------------------
function Window:GetDeviceInfo()
    local info=classifyDevice()
    self.DeviceInfo=info
    return info
end

function Window:IsMobile()
    return self:GetDeviceInfo().IsMobile
end

function Window:IsComputer()
    return self:GetDeviceInfo().IsComputer
end

function Window:IsConsole()
    return self:GetDeviceInfo().IsConsole
end

function Window:OnDeviceChanged(callback)
    self._v403DeviceListeners=self._v403DeviceListeners or {}
    local entry={Callback=callback,Connected=true}
    table.insert(self._v403DeviceListeners,entry)
    local connection={}
    function connection:Disconnect() entry.Connected=false end
    return connection
end

local function publishDevice(window,force)
    local nextInfo=classifyDevice()
    local changed=force or not sameDevice(window.DeviceInfo,nextInfo)
    window.DeviceInfo=nextInfo
    if window.ScreenGui then
        safeSetAttribute(window.ScreenGui,"AstraPlatform",nextInfo.Platform)
        safeSetAttribute(window.ScreenGui,"AstraFormFactor",nextInfo.FormFactor)
        safeSetAttribute(window.ScreenGui,"AstraOrientation",nextInfo.Orientation)
        safeSetAttribute(window.ScreenGui,"AstraLayout",nextInfo.Layout)
        safeSetAttribute(window.ScreenGui,"AstraPreferredInput",nextInfo.PreferredInput)
    end
    if changed then
        for _,entry in ipairs(window._v403DeviceListeners or {}) do
            if entry.Connected then safeCall(entry.Callback,nextInfo,window) end
        end
    end
    return nextInfo
end
...
-- Geometry / resize ------------------------------------------------------------
local function pxSize(value,defaultX,defaultY)
    ...
    if type(value)=="table" then return Vector2.new(tonumber(value.X or value[1]) or defaultX,tonumber(value.Y or value[2]) or defaultY) end
    return Vector2.new(defaultX,defaultY)
end

function Window:GetWindowSize()
    if not self.Root then return Vector2.zero end
    return Vector2.new(self.Root.AbsoluteSize.X,self.Root.AbsoluteSize.Y)
end

function Window:SetWindowSize(width,height,animated)
    if self.Destroyed or not self.Root then return false end
    local device=self:GetDeviceInfo()
    local requested
    if typeof(width)=="Vector2" then requested=width else requested=Vector2.new(tonumber(width) or self.Root.AbsoluteSize.X,tonumber(height) or self.Root.AbsoluteSize.Y) end
    local minSize=pxSize(self.Options.MinSize,560,360)
    local maxOpt=pxSize(self.Options.MaxSize,1100,760)
    local vp=viewportSize()
    local maxSize=Vector2.new(math.min(maxOpt.X,math.max(minSize.X,vp.X-20)),math.min(maxOpt.Y,math.max(minSize.Y,vp.Y-20)))
    local w=math.clamp(requested.X,minSize.X,maxSize.X)
    local h=math.clamp(requested.Y,minSize.Y,maxSize.Y)
    local target=UDim2.fromOffset(math.floor(w+0.5),math.floor(h+0.5))

    -- Phone portrait remains viewport-managed; update the remembered desktop/base
    -- size but do not fight the responsive drawer layout.
    self._v403UserSize=target
    self.Options.Size=target
    self._v32BaseSize=target
    if not (device.IsMobile and device.Orientation=="Portrait") then
        if animated==false then self.Root.Size=target else tween(self.Root,0.20,{Size=target},Enum.EasingStyle.Quart,Enum.EasingDirection.Out) end
    end
    task.delay(0.22,function()
        if not self.Destroyed then
            if self.RefreshSpacing then self:RefreshSpacing() end
            if self.RepairLayout then self:RepairLayout() end
        end
    end)
    return true,Vector2.new(w,h)
end

function Window:ResizeBy(deltaWidth,deltaHeight,animated)
    local current=self._v403UserSize or self._v32BaseSize or self.Root.Size
    local c=pxSize(current,self.Root.AbsoluteSize.X,self.Root.AbsoluteSize.Y)
    return self:SetWindowSize(c.X+(tonumber(deltaWidth) or 0),c.Y+(tonumber(deltaHeight) or 0),animated)
end

function Window:ResetWindowSize(animated)
    local base=self._v403DefaultSize or UDim2.fromOffset(840,550)
    return self:SetWindowSize(base.X.Offset,base.Y.Offset,animated)
end

function Window:SetWindowCornerRadius(radius)
    radius=math.clamp(tonumber(radius) or 18,0,40)
    self._v403WindowRadius=radius
    if self.Root then
        local c=self.Root:FindFirstChildOfClass("UICorner")
        if not c then c=Instance.new("UICorner"); c.Parent=self.Root end
        c.CornerRadius=UDim.new(0,radius)
    end
    return radius
end

local function applyRoundness(window,root)
    local factor=math.clamp(tonumber(window._v403Roundness) or 1,0,2)
    root=root or window.ScreenGui
    if not root then return end
    local function apply(c)
        if not c:IsA("UICorner") then return end
        local current=c.CornerRadius
        if current.Scale~=0 or current.Offset>=100 then return end -- preserve pills/circles
        local base=c:GetAttribute("AstraBaseCornerRadius")
        if type(base)~="number" then base=current.Offset; safeSetAttribute(c,"AstraBaseCornerRadius",base) end
        c.CornerRadius=UDim.new(0,math.clamp(math.floor(base*factor+0.5),0,48))
    end
    if root:IsA("UICorner") then apply(root) end
    for _,obj in ipairs(root:GetDescendants()) do apply(obj) end
    if window._v403WindowRadius then window:SetWindowCornerRadius(window._v403WindowRadius) end
end

function Window:SetRoundness(factor)
    self._v403Roundness=math.clamp(tonumber(factor) or 1,0,2)
    applyRoundness(self)
    return self._v403Roundness
end

function Window:GetRoundness()
    return self._v403Roundness or 1
end

function Window:SetInterfaceScale(value)
    if type(self.SetScale)=="function" then return self:SetScale(math.clamp(tonumber(value) or 1,0.65,1.35)) end
    return false
end

function Window:IncreaseInterfaceSize(step)
    step=tonumber(step) or 40
    return self:ResizeBy(step,math.floor(step*0.62),true)
end

function Window:DecreaseInterfaceSize(step)
    step=tonumber(step) or 40
    return self:ResizeBy(-step,-math.floor(step*0.62),true)
end

function Window:GetInterfaceMetrics()
    local d=self:GetDeviceInfo()
    return {
        Device=d,
        WindowSize=self:GetWindowSize(),
        Scale=self.Scale and self.Scale.Scale or 1,
        Roundness=self:GetRoundness(),
        WindowRadius=self._v403WindowRadius or 18,
        Resizable=self._v403Resizable==true,
    }
end

local function addResizeGrip(window)
    if not window.Root or window._v403ResizeGrip then return end
    local grip=Instance.new("TextButton")
    grip.Name="AstraResizeGrip"; grip.Text=""; grip.AutoButtonColor=false; grip.BackgroundTransparency=1
    grip.AnchorPoint=Vector2.new(1,1); grip.Position=UDim2.new(1,-4,1,-4); grip.Size=UDim2.fromOffset(28,28); grip.ZIndex=80; grip.Parent=window.Root
    window._v403ResizeGrip=grip

    for i=0,2 do
        local line=Instance.new("Frame")
        line.AnchorPoint=Vector2.new(1,1); line.Position=UDim2.new(1,-3-i*5,1,-3); line.Size=UDim2.fromOffset(1,7+i*5)
        line.Rotation=45; line.BackgroundColor3=window.Theme.TextSecondary or window.Theme.Muted; line.BackgroundTransparency=0.40+i*0.12
        line.BorderSizePixel=0; line.ZIndex=81; line.Parent=grip
    end
    ...
    local startSize
    local activeInput
    window:_connect(grip.InputBegan,function(io)
        if io.UserInputType==Enum.UserInputType.MouseButton1 or io.UserInputType==Enum.UserInputType.Touch then
            dragging=true; activeInput=io; startPoint=io.Position; startSize=window:GetWindowSize()
        end
    end)
    window:_connect(UIS.InputChanged,function(io)
        if not dragging then return end
        local valid=io.UserInputType==Enum.UserInputType.MouseMovement or io.UserInputType==Enum.UserInputType.Touch
        if valid then
            local delta=io.Position-startPoint
            window:SetWindowSize(startSize.X+delta.X,startSize.Y+delta.Y,false)
        end
    end)
    window:_connect(UIS.InputEnded,function(io)
        if dragging and (io==activeInput or io.UserInputType==Enum.UserInputType.MouseButton1 or io.UserInputType==Enum.UserInputType.Touch) then
            dragging=false; activeInput=nil
            if window.RefreshSpacing then window:RefreshSpacing() end
        end
    end)
end

local function updateGrip(window)
    local info=window.DeviceInfo or classifyDevice()
    if window._v403ResizeGrip then
        window._v403ResizeGrip.Visible=window._v403Resizable==true and info.IsComputer
    end
end

-- CreateWindow integration -----------------------------------------------------
local _V403CreateWindow=AstraUI.CreateWindow
function AstraUI:CreateWindow(options)
    options=options or {}
    local window=_V403CreateWindow(self,options)
    if not window then return window end

    window._v403DefaultSize=options.Size or window._v32BaseSize or UDim2.fromOffset(840,550)
    window._v403UserSize=window._v403DefaultSize
    window._v403Roundness=math.clamp(tonumber(options.Roundness) or 1,0,2)
    window._v403WindowRadius=math.clamp(tonumber(options.WindowCornerRadius or options.CornerRadius) or 18,0,40)
    window._v403Resizable=options.Resizable~=false
    window._v403DeviceListeners={}

    publishDevice(window,true)
    applyRoundness(window)
    window:SetWindowCornerRadius(window._v403WindowRadius)
    addResizeGrip(window)
    updateGrip(window)

    local function refreshEnvironment()
        if window.Destroyed then return end
        local info=publishDevice(window,false)
        updateGrip(window)
        task.defer(function()
            if window.Destroyed then return end
            applyRoundness(window)
    ...
    if workspace.CurrentCamera then
        window:_connect(workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"),refreshEnvironment)
    end
    pcall(function()
        window:_connect(UIS:GetPropertyChangedSignal("PreferredInput"),refreshEnvironment)
    end)
    if window.ScreenGui then
        window:_connect(window.ScreenGui.DescendantAdded,function(obj)
            if obj:IsA("UICorner") then task.defer(function() if obj.Parent and not window.Destroyed then applyRoundness(window,obj) end end) end
        end)
    end

    AstraUI.Version=V403_VERSION
    return window
end
...
-- ============================================================================
-- AstraUI V4.0.4 - Notification / Tab State / Resize Safety hotfix
-- Focus: compact notifications, size-independent tab selection, genuinely
-- useful small/large window ranges, and viewport-safe resizing.
-- ============================================================================
;(function()
local V404_VERSION = "4.0.4-layout-hotfix"

local function v404Copy(source)
    local out = {}
    for k,v in pairs(source or {}) do out[k] = v end
    return out
end

local function v404Viewport()
    local camera = workspace.CurrentCamera
    return camera and camera.ViewportSize or Vector2.new(1280,720)
end

local function v404PxSize(value, defaultX, defaultY)
    if typeof(value) == "Vector2" then return value end
    if typeof(value) == "UDim2" then return Vector2.new(value.X.Offset,value.Y.Offset) end
    if type(value) == "table" then
        return Vector2.new(
            tonumber(value.X or value[1]) or defaultX,
            tonumber(value.Y or value[2]) or defaultY
        )
    end
    return Vector2.new(defaultX,defaultY)
end

local function v404Scale(window)
    local scale = window and window.Scale and tonumber(window.Scale.Scale) or 1
    return math.clamp(scale or 1,0.45,1.6)
end

-- --------------------------------------------------------------------------
-- Notifications: fixed-height toast cards. The old implementation placed the
-- animated accent bar in the same AutomaticSize container as a UIListLayout,
-- which could make the accent participate in layout and blow the toast up to
-- a huge panel on some mobile scale factors.
-- --------------------------------------------------------------------------
function Window:Notify(options)
    options = v404Copy(options or {})
    if self.Destroyed or not self.NotificationHost then return nil end
    self._activeNotifications = self._activeNotifications or {}

    -- Keep V4 notification-center history.
    if self._v4Notifications then
        self._v4NotificationId = (self._v4NotificationId or 0) + 1
        local key = tostring(options.Title or "Notification") .. "\0" .. tostring(options.Content or options.Text or "")
        local found
        for _,entry in ipairs(self._v4Notifications) do
            if entry.Key == key then found = entry break end
        end
        if found then
            found.Count = (found.Count or 1) + 1
            found.Time = os.time()
    ...
    local device = self.GetDeviceInfo and self:GetDeviceInfo() or {IsMobile=false,Orientation="Landscape",Width=v404Viewport().X}
    local requestedWidth = tonumber(options.Width)
    local width
    if requestedWidth then
        width = math.clamp(requestedWidth,210,360)
    elseif device.IsMobile then
        if device.Orientation == "Portrait" then
            width = math.clamp((device.Width or v404Viewport().X) - 22,220,300)
        else
            width = math.clamp(math.floor((device.Width or v404Viewport().X) * 0.28),220,260)
        end
    else
        width = 318
    end
    self.NotificationHost.Size = UDim2.fromOffset(width,0)

    local actions = type(options.Actions) == "table" and options.Actions or nil
    local hasActions = actions and #actions > 0
    local contentText = tostring(options.Content or options.Text or "")
    local bodyHeight = (#contentText > 88) and 32 or 18
    local cardHeight = hasActions and (78 + bodyHeight) or (49 + bodyHeight)
    cardHeight = math.clamp(cardHeight,68,116)

    local card = create("Frame",{
        Parent=self.NotificationHost,
        Size=UDim2.new(1,0,0,cardHeight),
        BackgroundColor3=theme.Surface3 or theme.Surface,
        BackgroundTransparency=0,
        BorderSizePixel=0,
        ClipsDescendants=true,
        ZIndex=101,
    },{
        corner(math.max(10,math.floor(12*(self._v403Roundness or 1)))),
        stroke(theme.BorderDefault or theme.Border,0.36,1),
    })
    card.Name = "AstraNotification"

    local accentBar = create("Frame",{
        Name="AstraNotificationAccent",
        Parent=card,
    ...
    local title = create("TextLabel",{
        Parent=card,
        BackgroundTransparency=1,
        Position=UDim2.fromOffset(29,9),
        Size=UDim2.new(1,-65,0,19),
        Text=tostring(options.Title or "Notification"),
        TextColor3=theme.TextPrimary or theme.Text,
        TextXAlignment=Enum.TextXAlignment.Left,
        TextYAlignment=Enum.TextYAlignment.Center,
        TextSize=12,
        Font=Enum.Font.GothamBold,
    ...
    local close = create("TextButton",{
        ...
        BackgroundTransparency=1,
        Text="×",
        TextColor3=theme.TextSecondary or theme.Muted,
        TextSize=17,
        Font=Enum.Font.Gotham,
        ZIndex=104,
    })
    ...
    local body = create("TextLabel",{
        ...
        Size=UDim2.new(1,-26,0,bodyHeight),
        Text=contentText,
        TextColor3=theme.TextSecondary or theme.Muted,
        TextXAlignment=Enum.TextXAlignment.Left,
        TextYAlignment=Enum.TextYAlignment.Top,
    ...
    if hasActions then
        actionHolder = create("Frame",{
            Parent=card,
            Position=UDim2.new(0,13,1,-37),
            Size=UDim2.new(1,-26,0,29),
            BackgroundTransparency=1,
            ZIndex=103,
        })
        create("UIListLayout",{
    ...
    local scale = card:FindFirstChild("AstraNotificationMotion")
    if not scale then
        scale = Instance.new("UIScale")
        scale.Name = "AstraNotificationMotion"
        scale.Parent = card
    end

    local state = collectTransparency(card)
    local closed = false
    local handle = {Card=card}
    ...
    local function closeToast()
        ...
        tween(scale,0.16,{Scale=0.94},Enum.EasingStyle.Quart,Enum.EasingDirection.In)
        tween(accentBar,0.13,{Size=UDim2.fromOffset(3,0)},Enum.EasingStyle.Quart,Enum.EasingDirection.In)
        tween(card,0.17,{BackgroundTransparency=1},Enum.EasingStyle.Quart,Enum.EasingDirection.In)
        tweenTransparency(state,0.14,true)
        task.delay(0.18,destroyCard)
    end

    handle.Close = closeToast
    function handle:Update(data)
        data = data or {}
        if data.Title ~= nil then title.Text = tostring(data.Title) end
        if data.Content ~= nil or data.Text ~= nil then body.Text = tostring(data.Content or data.Text or "") end
    ...
    self:_connect(close.MouseButton1Click,closeToast)
    ...
    if actionHolder then
        for _,action in ipairs(actions) do
            local actionButton = button(
                ...
                UDim2.fromOffset(math.clamp(tonumber(action.Width) or 72,58,96),27),
                action.Primary and accent or (theme.SurfaceInteractive or theme.Surface2)
            )
            actionButton.TextSize = 10
            actionButton.ZIndex = 104
            self:_connect(actionButton.MouseButton1Click,function()
                safeCall(action.Callback,handle)
                if action.Close ~= false then closeToast() end
            end)
        end
        state = collectTransparency(card)
    end

    table.insert(self._activeNotifications,handle)
    local maxNotifications = math.clamp(tonumber(self.Options.MaxNotifications) or 4,1,6)
    while #self._activeNotifications > maxNotifications do
        local oldest = self._activeNotifications[1]
        if oldest and oldest.Close then oldest.Close() else table.remove(self._activeNotifications,1) end
    end

    -- Compact entrance: visible, but never expands the layout.
    scale.Scale = 0.94
    card.BackgroundTransparency = 1
    applyHidden(state)
    ...
    task.defer(function()
        if closed or not card.Parent then return end
        tween(scale,0.21,{Scale=1},Enum.EasingStyle.Back,Enum.EasingDirection.Out)
        tween(card,0.16,{BackgroundTransparency=0},Enum.EasingStyle.Quart,Enum.EasingDirection.Out)
        tweenTransparency(state,0.16,false)
        tween(accentBar,0.20,{Size=UDim2.fromOffset(3,cardHeight-14)},Enum.EasingStyle.Quart,Enum.EasingDirection.Out)
    end)
    ...
    if duration > 0 then
        tween(fill,duration,{Size=UDim2.fromScale(0,1)},Enum.EasingStyle.Linear,Enum.EasingDirection.Out)
        task.delay(duration,closeToast)
    else
        progress.Visible = false
    end
...
-- --------------------------------------------------------------------------
-- Tabs: selection no longer uses an external Accent outline. The outline was
-- visually unstable when the sidebar changed width or roundness. Selection is
-- now communicated by a soft fill + left indicator + icon/text color.
-- --------------------------------------------------------------------------
local function v404StyleTabs(window)
    if not window or window.Destroyed then return end
    local theme = window.Theme
    for _,tab in ipairs(window.Tabs or {}) do
        local selected = tab == window.CurrentTab
        ...
        if btn then
            btn.BackgroundColor3 = selected and (theme.AccentSoft or theme.Surface2) or theme.Surface
            btn.BackgroundTransparency = selected and 0.12 or 1
            btn.ClipsDescendants = true

            for _,child in ipairs(btn:GetChildren()) do
                if child:IsA("UIStroke") and string.find(child.Name,"TabStroke",1,true) then
                    child.Transparency = 1
                end
            end

            local pulse = btn:FindFirstChild("AstraV371TabPulse")
            if pulse and pulse:IsA("Frame") then
                pulse.BackgroundTransparency = 1
            end
            local selectedScale = btn:FindFirstChild("AstraV371SelectedPulse")
            if selectedScale and selectedScale:IsA("UIScale") then
        ...
        if tab.Indicator then
            ...
            tab.Indicator.Position = UDim2.new(0,4,0.5,0)
            tab.Indicator.Size = UDim2.fromOffset(3,selected and 20 or 14)
            tab.Indicator.BackgroundColor3 = theme.Accent
            tab.Indicator.BackgroundTransparency = selected and 0 or 1
        end

        if tab.ButtonText then
            tab.ButtonText.TextColor3 = selected and (theme.TextPrimary or theme.Text) or (theme.TextSecondary or theme.Muted)
        end

        if tab.IconHost then
            for _,obj in ipairs(tab.IconHost:GetDescendants()) do
                if obj:IsA("Frame") then
                    local key = obj:GetAttribute("AstraIconColorKey")
                    if key then obj.BackgroundColor3 = selected and theme.Accent or (theme.TextSecondary or theme.Muted) end
                elseif obj:IsA("ImageLabel") then
                    obj.ImageColor3 = selected and theme.Accent or (theme.TextSecondary or theme.Muted)
                end
            end
        end
    end
...
local _V404CreateTab = Window.CreateTab
function Window:CreateTab(options)
    local tab = _V404CreateTab(self,options)
    task.defer(function()
        if not self.Destroyed then v404StyleTabs(self) end
    end)
    return tab
end

local _V404SelectTab = Window.SelectTab
function Window:SelectTab(tab)
    local result = _V404SelectTab(self,tab)
    task.defer(function()
        if not self.Destroyed then v404StyleTabs(self) end
    end)
    return result
...
local _V404SetTheme = Window.SetTheme
function Window:SetTheme(themePatch)
    local result = _V404SetTheme(self,themePatch)
    task.defer(function()
        if not self.Destroyed then v404StyleTabs(self) end
    end)
    return result
...
-- --------------------------------------------------------------------------
-- Window sizing: a much smaller true minimum, a usable safe maximum, presets,
-- and viewport clamping. Logical limits account for UIScale so the rendered
-- window never pushes its topbar outside the visible viewport.
-- --------------------------------------------------------------------------
function Window:GetWindowSizeLimits()
    local device = self.GetDeviceInfo and self:GetDeviceInfo() or {IsMobile=false,Orientation="Landscape"}
    local vp = v404Viewport()
    local scale = v404Scale(self)
    local marginX = device.IsMobile and 10 or 18
    local marginY = device.IsMobile and 10 or 18

    local minOpt = v404PxSize(self.Options.MinSize,460,300)
    local maxOpt = v404PxSize(self.Options.MaxSize,1400,900)

    local fitX = math.max(360,(vp.X - marginX*2) / scale)
    local fitY = math.max(260,(vp.Y - marginY*2) / scale)

    local maxSize = Vector2.new(
        math.max(360,math.min(maxOpt.X,fitX)),
        math.max(260,math.min(maxOpt.Y,fitY))
    )
    local minSize = Vector2.new(
        math.min(math.max(360,minOpt.X),maxSize.X),
        math.min(math.max(260,minOpt.Y),maxSize.Y)
    )

    return {Min=minSize,Max=maxSize,Viewport=vp,Scale=scale}
end

function Window:KeepWindowAccessible(animated)
    if self.Destroyed or not self.Root then return false end
    local vp = v404Viewport()
    local scale = v404Scale(self)
    local displayed = Vector2.new(self.Root.AbsoluteSize.X*scale,self.Root.AbsoluteSize.Y*scale)
    local halfW, halfH = displayed.X*0.5, displayed.Y*0.5
    local margin = 10
    ...
    if displayed.X >= vp.X-margin*2 then
        cx = vp.X*0.5
    else
        cx = math.clamp(cx,margin+halfW,vp.X-margin-halfW)
    end
    if displayed.Y >= vp.Y-margin*2 then
        cy = vp.Y*0.5
    else
        cy = math.clamp(cy,margin+halfH,vp.Y-margin-halfH)
    end

    local target = UDim2.new(0.5,cx-vp.X*0.5,0.5,cy-vp.Y*0.5)
    if animated == false then self.Root.Position = target else tween(self.Root,0.16,{Position=target},Enum.EasingStyle.Quart,Enum.EasingDirection.Out) end
    return true
end
...
function Window:SetWindowSize(width,height,animated)
    if self.Destroyed or not self.Root then return false end
    local current = self._v403UserSize or self._v32BaseSize or self.Root.Size
    local cur = v404PxSize(current,self.Root.AbsoluteSize.X,self.Root.AbsoluteSize.Y)
    local requested = typeof(width)=="Vector2" and width or Vector2.new(tonumber(width) or cur.X,tonumber(height) or cur.Y)
    local limits = self:GetWindowSizeLimits()

    local w = math.clamp(requested.X,limits.Min.X,limits.Max.X)
    local h = math.clamp(requested.Y,limits.Min.Y,limits.Max.Y)
    local target = UDim2.fromOffset(math.floor(w+0.5),math.floor(h+0.5))
    ...
    if not (device.IsMobile and device.Orientation == "Portrait") then
        if animated == false then
            ...
            tween(self.Root,0.20,{Size=target},Enum.EasingStyle.Quart,Enum.EasingDirection.Out)
        end
    end

    task.delay(animated == false and 0.02 or 0.21,function()
        if self.Destroyed then return end
        self:KeepWindowAccessible(false)
        if self.RefreshSpacing then self:RefreshSpacing() end
        if self.RepairLayout then self:RepairLayout() end
    end)
...
function Window:ResizeBy(deltaWidth,deltaHeight,animated)
    local current = self._v403UserSize or self._v32BaseSize or self.Root.Size
    local c = v404PxSize(current,self.Root.AbsoluteSize.X,self.Root.AbsoluteSize.Y)
    return self:SetWindowSize(c.X+(tonumber(deltaWidth) or 0),c.Y+(tonumber(deltaHeight) or 0),animated)
end

function Window:SetWindowSizePreset(name,animated)
    name = string.lower(tostring(name or "normal"))
    local limits = self:GetWindowSizeLimits()
    local def = v404PxSize(self._v403DefaultSize or self.Options.Size,840,550)
    local target

    if name == "small" or name == "minimum" or name == "min" then
        target = Vector2.new(
            math.min(limits.Max.X,limits.Min.X + 20),
            math.min(limits.Max.Y,limits.Min.Y + 16)
        )
    elseif name == "large" then
        target = Vector2.new(
            math.min(limits.Max.X,math.max(def.X*1.18,def.X+100)),
            math.min(limits.Max.Y,math.max(def.Y*1.16,def.Y+70))
        )
    elseif name == "max" or name == "maximum" then
        target = limits.Max
    else
        target = Vector2.new(
            math.clamp(def.X,limits.Min.X,limits.Max.X),
            math.clamp(def.Y,limits.Min.Y,limits.Max.Y)
        )
    end

    return self:SetWindowSize(target,nil,animated)
end

function Window:ResetWindowSize(animated)
    return self:SetWindowSizePreset("normal",animated)
end

function Window:IncreaseInterfaceSize(step)
    step = tonumber(step) or 80
    return self:ResizeBy(step,math.floor(step*0.62),true)
end
...
local _V404CreateWindow = AstraUI.CreateWindow
function AstraUI:CreateWindow(options)
    options = options or {}
    if options.MinSize == nil then options.MinSize = Vector2.new(460,300) end
    if options.MaxSize == nil then options.MaxSize = Vector2.new(1400,900) end

    local window = _V404CreateWindow(self,options)
    if not window then return window end

    task.defer(function()
        if window.Destroyed then return end
        v404StyleTabs(window)
        window:KeepWindowAccessible(false)
    end)
    ...
    if workspace.CurrentCamera then
        window:_connect(workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"),function()
            task.defer(function()
                if window.Destroyed then return end
                local current = window._v403UserSize
                if current then
                    local p = v404PxSize(current,840,550)
                    window:SetWindowSize(p.X,p.Y,false)
                end
                window:KeepWindowAccessible(false)
            end)
        end)
