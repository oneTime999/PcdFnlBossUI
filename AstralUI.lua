-- AstraUI V5.2.1 - Signal Core Fix / Mobile Interaction Recovery
...
function Window:_responsive()
    ...
    local sw
    if collapsed then
        sw=mobile and 56 or 62
    else
        sw=self.CurrentSize.X<860 and 188 or 204
    end
...
local function preset(w,name)
    ...
    if w:IsMobile() then
        if w.DeviceInfo.Orientation=="Portrait" then
            ...
            p={
                small=Vector2.new(math.min(560,sm.X),math.min(300,sm.Y)),
                normal=Vector2.new(normalW,normalH),
                large=Vector2.new(math.min(790,sm.X),math.min(380,sm.Y)),
                max=sm,
            }
        end
    else
        p={
            small=Vector2.new(620,400),
...
function Window:Notify(o)
    ...
    local targetWidth=mobile and 292 or 328
    local width=math.max(220,math.min(targetWidth,v.X-24))
    local ch=content~="" and textHeight(content,10,width-50) or 0
    local height=math.floor(clamp(46+(ch>0 and math.min(ch,34)+8 or 0),54,mobile and 92 or 102))
    ...
    tt.Position=UDim2.fromOffset(14,8)
    tt.Size=UDim2.new(1,-46,0,18)
    ...
    local body=label(f,content,10,t.TextSecondary)
    body.Position=UDim2.fromOffset(14,28)
    body.Size=UDim2.new(1,-28,0,math.max(17,math.min(ch,34)))
...
function Window:CreateTab(o)
    ...
    local p=create("ScrollingFrame",{Parent=self.Pages,Size=UDim2.fromScale(1,1),BackgroundTransparency=1,BorderSizePixel=0,Visible=false,CanvasSize=UDim2.new(),AutomaticCanvasSize=Enum.AutomaticSize.Y,ScrollBarThickness=3,ScrollBarImageColor3=self.Theme.Scrollbar});padding(p,18,16,18,28);list(p,14,false);tab.Page=p;table.insert(self.Tabs,tab);self:_connect(b.Activated,function()self:SelectTab(tab)end);self:_registerSearch({Instance=b,Haystack=string.lower(tab.Name.." "..tab.Description),Type="Tab",Object=tab});tab:_sidebar(self.SidebarCollapsed);if#self.Tabs==1 then self:SelectTab(tab)end;return tab
end
...
local function card(s,name,desc,h)
    ...
    local r=create("Frame",{
        Parent=s.Frame,
        Size=UDim2.new(1,0,0,h or 68),
        BackgroundColor3=t.Surface2,
        BorderSizePixel=0,
    })
    r:SetAttribute("AstraBaseRadius",10)
    corner(r,10*w.Roundness)
    stroke(r,t.BorderSubtle,.42,1)

    local a=label(r,name,12,t.TextPrimary,Enum.Font.GothamMedium)
    a.Position=UDim2.fromOffset(14,9)
    a.Size=UDim2.new(1,-28,0,20)
    a.TextTruncate=Enum.TextTruncate.AtEnd

    local d=label(r,desc or "",10,t.TextSecondary)
    ...
    function c:SetActionReserve(px)
        ...
        self.Title.Size=UDim2.new(1,-(28+px),0,20)
        self.Description.Size=UDim2.new(1,-(28+px),0,18)
    end
...
function Section:AddButton(o)
    o=o or {};local r,c=card(self,o.Name or "Button",o.Description,64);local w=self.Window;local b=button(r,o.ButtonText or o.Text or o.Name or "Run",w.Theme,o.Style or "Primary");b.AnchorPoint=Vector2.new(1,.5);b.Position=UDim2.new(1,-14,.5,0);b.Size=UDim2.fromOffset(clamp(textWidth(b.Text,12)+34,82,150),36);c.Button=b;c:SetActionReserve(b.Size.X.Offset+18)
...
function Section:AddToggle(o)
    o=o or {};local def=not not o.Default;local r,c=card(self,o.Name or "Toggle",o.Description,64);local w=self.Window;local tr=create("TextButton",{Parent=r,AnchorPoint=Vector2.new(1,.5),Position=UDim2.new(1,-14,.5,0),Size=UDim2.fromOffset(54,28),BackgroundColor3=w.Theme.Surface3,BorderSizePixel=0,Text="",AutoButtonColor=false});corner(tr,14);local k=create("Frame",{Parent=tr,AnchorPoint=Vector2.new(.5,.5),Position=UDim2.fromOffset(def and 40 or 14,14),Size=UDim2.fromOffset(20,20),BackgroundColor3=def and Color3.new(1,1,1)or w.Theme.TextSecondary,BorderSizePixel=0});corner(k,99);local val=def;c:SetActionReserve(76)
    local function apply(v,fire)val=not not v;tr.BackgroundColor3=val and w.Theme.Accent or w.Theme.Surface3;k.BackgroundColor3=val and Color3.new(1,1,1)or w.Theme.TextSecondary;tween(k,.14,{Position=UDim2.fromOffset(val and 40 or 14,14)});w:_setFlag(o.Flag,val,c);if fire~=false then safeCall(o.Callback,val)end end
...
function Section:AddDropdown(o)
    o=o or {};local multi=o.Multi==true;local def=o.Default;if multi and type(def)~="table"then def={}elseif not multi and def==nil then def=(o.Options or {})[1]end;local r,c=card(self,o.Name or "Dropdown",o.Description,72);local w=self.Window;local b=button(r,"",w.Theme,"Secondary");b.AnchorPoint=Vector2.new(1,.5);b.Position=UDim2.new(1,-14,.5,0);b.Size=UDim2.fromOffset(240,38);local val=multi and copy(def)or def;c:SetActionReserve(270)
