-- AstraUI V5.4.0 - Adaptive Layout Engine / Stable Interaction Core
...
function Window:Notify(o)
    ...
    local targetWidth=mobile and 272 or 308
    local width=math.max(210,math.min(targetWidth,v.X-20))
    local ch=content~="" and textHeight(content,10,width-50) or 0
    local height=math.floor(clamp(42+(ch>0 and math.min(ch,30)+7 or 0),50,mobile and 82 or 88))
    ...
    local function close()
        ...
        rec.Closed=true
        tween(sc,.12,{Scale=.96},Enum.EasingStyle.Quad,Enum.EasingDirection.In)
        tween(f,.18,{Position=UDim2.new(1,width+14,0,0),BackgroundTransparency=.12},Enum.EasingStyle.Quart,Enum.EasingDirection.In)
        tween(tt,.10,{TextTransparency=1})
        tween(body,.10,{TextTransparency=1})
        tween(closeButton,.10,{TextTransparency=1})
        tween(stripe,.10,{BackgroundTransparency=1})
        tween(progress,.10,{BackgroundTransparency=1})
    ...
    rec.Close=close
    self:_connect(closeButton.Activated,close)

    tween(sc,.20,{Scale=1},Enum.EasingStyle.Back,Enum.EasingDirection.Out)
    tween(f,.22,{Position=UDim2.new(1,0,0,0)},Enum.EasingStyle.Quart,Enum.EasingDirection.Out)

    if not persistent then
        tween(progress,duration,{Size=UDim2.new(0,0,0,2)},Enum.EasingStyle.Linear,Enum.EasingDirection.InOut)
        task.delay(duration,function()
            if not self.Destroyed then close() end
        end)
    end
