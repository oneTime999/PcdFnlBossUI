...
-- ============================================================================
-- AstraUI V3.7 premium motion + interaction polish
-- Focus: subtle visual effects, tactile feedback and state transitions without
-- changing the established V3.6 component API or responsive behavior.
-- ============================================================================

local V37_VERSION = "3.7.0-executor"

local function v37EffectsEnabled(window)
    if not window or window.Destroyed then return false end
    local options = window.Options or {}
    if options.Effects == false or options.ReduceMotion == true then
        return false
    end
    return true
end

local function v37EnsureScale(gui, name)
    if not gui then return nil end
    name = name or "AstraV37Scale"
...
local function v37EnsureStroke(gui, name, color, transparency, thickness)
    if not gui then return nil end
    local existing = gui:FindFirstChild(name)
    ...
    outline.Color = color or Color3.new(1,1,1)
    outline.Transparency = transparency == nil and 1 or transparency
    outline.Thickness = thickness or 1
    outline.Parent = gui
    return outline
end

local function v37Connect(window, signal, callback)
...
local function v37AnimateScale(window, gui, hoverScale, pressedScale)
    if not gui or gui:GetAttribute("AstraV37ScaleEffect") then return end
    gui:SetAttribute("AstraV37ScaleEffect", true)
    local scale = v37EnsureScale(gui)
    local hovering = false
    local pressing = false

    local function paint()
        if not v37EffectsEnabled(window) then
            scale.Scale = 1
            return
        end
        local target = pressing and (pressedScale or 0.98) or (hovering and (hoverScale or 1.015) or 1)
        tween(scale, pressing and 0.07 or 0.12, {Scale = target}, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
    end

    v37Connect(window, gui.MouseEnter, function()
        hovering = true
        paint()
    end)
    v37Connect(window, gui.MouseLeave, function()
        hovering = false
        pressing = false
        paint()
    end)
    v37Connect(window, gui.InputBegan, function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1
        or input.UserInputType == Enum.UserInputType.Touch then
            pressing = true
            paint()
    ...
    v37Connect(window, gui.InputEnded, function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1
        or input.UserInputType == Enum.UserInputType.Touch then
            pressing = false
            paint()
...
local function v37AddShine(window, gui)
    if not gui or gui:GetAttribute("AstraV37ShineEffect") then return end
    gui:SetAttribute("AstraV37ShineEffect", true)
    gui.ClipsDescendants = true

    local shine = Instance.new("Frame")
    shine.Name = "AstraV37Shine"
    ...
    shine.Active = false
    shine.ZIndex = math.max(gui.ZIndex + 1, 2)
    shine.Parent = gui

    local running = false
    local function sweep()
        if running or not v37EffectsEnabled(window) or not shine.Parent then return end
        running = true
        shine.Position = UDim2.fromScale(-0.25,0.5)
        tween(shine, 0.42, {Position = UDim2.fromScale(1.25,0.5)}, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        task.delay(0.44, function()
            if shine and shine.Parent then shine.Position = UDim2.fromScale(-0.25,0.5) end
            running = false
        end)
    end

    v37Connect(window, gui.MouseEnter, sweep)
end

local function v37DecorateRow(window, row)
    if not row or row:GetAttribute("AstraV37RowEffect") then return end
    row:SetAttribute("AstraV37RowEffect", true)

    local focusStroke = v37EnsureStroke(row, "AstraV37FocusStroke", window.Theme.Accent, 1, 1)
    focusStroke:SetAttribute("AstraV37ColorKey", "Accent")
    local rowScale = v37EnsureScale(row, "AstraV37RowScale")

    if v37EffectsEnabled(window) then
        rowScale.Scale = 0.988
        task.defer(function()
            if rowScale.Parent and not window.Destroyed then
                tween(rowScale, 0.18, {Scale = 1}, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
            end
        end)
    end
    ...
    local function paint()
        ...
        local targetScale = pressing and 0.996 or (hovering and 1.0025 or 1)
        tween(rowScale, pressing and 0.06 or 0.11, {Scale = targetScale})
        tween(focusStroke, 0.12, {Transparency = hovering and 0.70 or 1})
    end

    v37Connect(window,row.MouseEnter,function() hovering=true; paint() end)
    v37Connect(window,row.MouseLeave,function() hovering=false; pressing=false; paint() end)
    v37Connect(window,row.InputBegan,function(input)
        if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
            pressing=true; paint()
        end
    end)
    v37Connect(window,row.InputEnded,function(input)
        if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
            pressing=false; paint()
        end
    end)
end

local function v37RefreshThemeEffects(window)
    ...
    for _, descendant in ipairs(window.ScreenGui:GetDescendants()) do
        if descendant:IsA("UIStroke") then
            local key = descendant:GetAttribute("AstraV37ColorKey")
            if key and window.Theme[key] then
                descendant.Color = window.Theme[key]
            end
        elseif descendant:IsA("UIGradient") and descendant:GetAttribute("AstraV37AccentGradient") then
            descendant.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, window.Theme.Accent),
                ColorSequenceKeypoint.new(1, window.Theme.Accent2 or window.Theme.Accent),
            })
        end
    end
end

local function v37DecorateSearch(window)
    local searchBox = window.SearchBox
    local holder = searchBox and searchBox.Parent
    if not searchBox or not holder or holder:GetAttribute("AstraV37SearchEffect") then return end
    holder:SetAttribute("AstraV37SearchEffect",true)
    local glow = v37EnsureStroke(holder,"AstraV37SearchGlow",window.Theme.Accent,1,1)
    glow:SetAttribute("AstraV37ColorKey","Accent")
    local scale = v37EnsureScale(holder,"AstraV37SearchScale")

    v37Connect(window,searchBox.Focused,function()
        if not v37EffectsEnabled(window) then return end
        tween(glow,0.14,{Transparency=0.32})
        tween(scale,0.14,{Scale=1.008})
    end)
    v37Connect(window,searchBox.FocusLost,function()
        tween(glow,0.14,{Transparency=1})
        tween(scale,0.14,{Scale=1})
    end)
end

local function v37FindBrandIcon(window)
    if not window.Sidebar then return nil end
    for _, descendant in ipairs(window.Sidebar:GetDescendants()) do
        if descendant:IsA("Frame") and descendant.Size.X.Offset >= 34 and descendant.Size.X.Offset <= 44
        and descendant.Size.Y.Offset >= 34 and descendant.Size.Y.Offset <= 44 then
            local label = descendant:FindFirstChildOfClass("TextLabel")
            if label and label.Text == tostring(window.Options.IconText or "A") then
                return descendant
            end
        end
    end
...
local function v37DecorateBrand(window)
    local icon = v37FindBrandIcon(window)
    if not icon or icon:GetAttribute("AstraV37BrandEffect") then return end
    icon:SetAttribute("AstraV37BrandEffect",true)
    local gradient = Instance.new("UIGradient")
    gradient.Name = "AstraV37BrandGradient"
    gradient.Rotation = 28
    -- UIGradient multiplies the parent's BackgroundColor3. Keeping this close
    -- to white preserves the selected accent while adding only a soft light falloff.
    gradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0,Color3.new(1,1,1)),
        ColorSequenceKeypoint.new(1,Color3.fromRGB(225,225,238)),
    })
    gradient.Parent = icon
    v37AnimateScale(window,icon,1.035,0.97)
end

local function v37DecorateTopActions(window)
    for _, control in ipairs({window.SidebarButton,window.ThemeButton,window.MinimizeButton,window.CloseButton,window.MobileOpen}) do
        if control and control:IsA("GuiObject") then
            v37AnimateScale(window,control,1.045,0.92)
            local glow = v37EnsureStroke(control,"AstraV37ActionGlow",window.Theme.Accent,1,1)
            glow:SetAttribute("AstraV37ColorKey","Accent")
            v37Connect(window,control.MouseEnter,function()
                if v37EffectsEnabled(window) then tween(glow,0.12,{Transparency=0.72}) end
            end)
            v37Connect(window,control.MouseLeave,function()
                tween(glow,0.12,{Transparency=1})
            end)
        end
    end
end

local function v37WrapWindowRoot(window)
    if not window.Root or window.EffectGroup then return end
    local parent = window.Root.Parent
    if not parent then return end

    local group = Instance.new("CanvasGroup")
    group.Name = "AstraV37WindowEffects"
    group.BackgroundTransparency = 1
    group.Size = UDim2.fromScale(1,1)
    group.Position = UDim2.fromScale(0,0)
    group.GroupTransparency = 0
    group.Parent = parent
    window.Root.Parent = group
    window.EffectGroup = group

    if v37EffectsEnabled(window) then
        local targetScale = window.Scale.Scale
        group.GroupTransparency = 0.16
        window.Scale.Scale = targetScale * 0.975
        task.defer(function()
            if window.Destroyed or not group.Parent then return end
            local finalScale = window.ResponsiveScale or targetScale or 1
            tween(group,0.20,{GroupTransparency=0},Enum.EasingStyle.Quart,Enum.EasingDirection.Out)
            tween(window.Scale,0.22,{Scale=finalScale},Enum.EasingStyle.Back,Enum.EasingDirection.Out)
        end)
    end
end

local _AstraV37Row = Tab._row
function Tab:_row(parent,height)
    local row = _AstraV37Row(self,parent,height)
    v37DecorateRow(self.Window,row)
    return row
end

local _AstraV37Button = Tab._addButton
function Tab:_addButton(parent,data)
    local object = _AstraV37Button(self,parent,data)
    local row = object and object.Instance
    if row then
        for _, child in ipairs(row:GetChildren()) do
            if child:IsA("TextButton") and child.AnchorPoint.X > 0.9 then
                v37AnimateScale(self.Window,child,1.025,0.94)
                v37AddShine(self.Window,child)
                local glow=v37EnsureStroke(child,"AstraV37ButtonGlow",self.Window.Theme.Accent,0.82,1)
                glow:SetAttribute("AstraV37ColorKey","Accent")
                break
            end
        end
...
local _AstraV37Toggle = Tab._addToggle
function Tab:_addToggle(parent,data)
    local object = _AstraV37Toggle(self,parent,data)
    local row = object and object.Instance
    if not row then return object end
    ...
    local glow=v37EnsureStroke(switch,"AstraV37ToggleGlow",self.Window.Theme.Accent,1,1.2)
    glow:SetAttribute("AstraV37ColorKey","Accent")
    local knobScale=knob and v37EnsureScale(knob,"AstraV37ToggleKnobScale")

    local function apply(pop)
        local on = object.Get and object:Get() == true
        if v37EffectsEnabled(self.Window) then
            tween(glow,0.14,{Transparency=on and 0.36 or 1})
            if pop and knobScale then
                knobScale.Scale=0.88
                tween(knobScale,0.18,{Scale=1},Enum.EasingStyle.Back,Enum.EasingDirection.Out)
            end
        else
            glow.Transparency=1
            if knobScale then knobScale.Scale=1 end
        end
    end

    self.Window:_connect(switch.MouseButton1Click,function()
        task.defer(function() if row.Parent then apply(true) end end)
    end)

    if object.Set then
        local oldSet=object.Set
        function object:Set(value,...)
            local result=oldSet(self,value,...)
            task.defer(function() if row.Parent then apply(true) end end)
            return result
        end
    end
    if object._sync then
        local oldSync=object._sync
        function object:_sync(value,...)
            local result=oldSync(self,value,...)
            task.defer(function() if row.Parent then apply(true) end end)
            return result
        end
    end
...
local _AstraV37Slider = Tab._addSlider
function Tab:_addSlider(parent,data)
    local object = _AstraV37Slider(self,parent,data)
    local row = object and object.Instance
    if not row then return object end
    ...
    local knob
    for _,child in ipairs(row:GetChildren()) do
        if child:IsA("TextButton") and child.Size.X.Scale > 0.5 and child.Size.Y.Offset <= 16 then
            bar=child
            for _,sub in ipairs(child:GetChildren()) do
                if sub:IsA("Frame") and sub.AnchorPoint.X == 0.5 and sub.Size.X.Offset <= 20 then knob=sub break end
            end
            break
        end
    end
    ...
    local halo=v37EnsureStroke(knob,"AstraV37SliderHalo",self.Window.Theme.Accent,0.72,2)
    halo:SetAttribute("AstraV37ColorKey","Accent")
    local scale=v37EnsureScale(knob,"AstraV37SliderScale")
    local active=false

    local function setActive(state)
        active=state==true
        if not v37EffectsEnabled(self.Window) then halo.Transparency=0.72; scale.Scale=1; return end
        tween(halo,0.10,{Transparency=active and 0.18 or 0.72})
        tween(scale,0.10,{Scale=active and 1.08 or 1})
    end
    self.Window:_connect(bar.MouseEnter,function() if not UserInputService.TouchEnabled then setActive(true) end end)
    self.Window:_connect(bar.MouseLeave,function() if not active then return end; setActive(false) end)
    self.Window:_connect(bar.InputBegan,function(input)
        if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then setActive(true) end
    end)
    self.Window:_connect(UserInputService.InputEnded,function(input)
        if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then setActive(false) end
    end)
    return object
end

local _AstraV37Dropdown = Tab._addDropdown
function Tab:_addDropdown(parent,data)
    local object = _AstraV37Dropdown(self,parent,data)
    local holder = object and object.Instance
    if not holder then return object end
    ...
    for _,child in ipairs(header:GetChildren()) do
        ...
        if child:IsA("TextLabel") and child.BackgroundTransparency < 1 and child.AnchorPoint.X > 0.9 then display=child end
    end
    if display then v37AnimateScale(self.Window,display,1.008,0.985) end

    if optionsFrame then
        local panelGlow=v37EnsureStroke(holder,"AstraV37DropdownGlow",self.Window.Theme.Accent,1,1)
        panelGlow:SetAttribute("AstraV37ColorKey","Accent")
        local function refreshOpenEffect()
            local open=optionsFrame.Visible
            if chevronHost and v37EffectsEnabled(self.Window) then
                tween(chevronHost,0.16,{Rotation=open and 180 or 0},Enum.EasingStyle.Quart,Enum.EasingDirection.Out)
            elseif chevronHost then
                chevronHost.Rotation=open and 180 or 0
            end
            tween(panelGlow,0.14,{Transparency=open and 0.70 or 1})
        end
        self.Window:_connect(header.MouseButton1Click,function() task.defer(refreshOpenEffect) end)
        self.Window:_connect(optionsFrame:GetPropertyChangedSignal("Visible"),refreshOpenEffect)
        for _,item in ipairs(optionsFrame:GetChildren()) do
            if item:IsA("TextButton") then v37AnimateScale(self.Window,item,1.004,0.975) end
        end
        refreshOpenEffect()
    end
    return object
...
local _AstraV37CreateTab = Window.CreateTab
function Window:CreateTab(options)
    local tab = _AstraV37CreateTab(self,options)
    if tab and tab.Button then
        v37AnimateScale(self,tab.Button,1.012,0.975)
        local glow=v37EnsureStroke(tab.Button,"AstraV37TabGlow",self.Theme.Accent,1,1)
        glow:SetAttribute("AstraV37ColorKey","Accent")
        tab._v37Glow=glow
    end

    if tab and tab.Page and not tab._v37Canvas then
        local oldParent=tab.Page.Parent
        if oldParent then
            local group=Instance.new("CanvasGroup")
            group.Name="AstraV37PageGroup"
            group.BackgroundTransparency=1
            group.Size=UDim2.fromScale(1,1)
            group.Position=UDim2.fromScale(0,0)
            group.GroupTransparency=0
            ...
            tab.Page.Parent=group
            tab._v37Canvas=group
        end
    end
    return tab
end

local _AstraV37SelectTab = Window.SelectTab
function Window:SelectTab(tab)
    _AstraV37SelectTab(self,tab)
    for _,other in ipairs(self.Tabs or {}) do
        if other._v37Glow then
            tween(other._v37Glow,0.14,{Transparency=(other==tab) and 0.68 or 1})
        end
    end
    if tab and tab._v37Canvas and v37EffectsEnabled(self) then
        tab._v37Canvas.GroupTransparency=0.18
        tween(tab._v37Canvas,0.18,{GroupTransparency=0},Enum.EasingStyle.Quart,Enum.EasingDirection.Out)
    elseif tab and tab._v37Canvas then
        tab._v37Canvas.GroupTransparency=0
    end
end

local _AstraV37CreateSection = Tab.CreateSection
function Tab:CreateSection(options)
    local section = _AstraV37CreateSection(self,options)
    if section and section.Frame and v37EffectsEnabled(self.Window) then
        local scale=v37EnsureScale(section.Frame,"AstraV37SectionScale")
        scale.Scale=0.992
        task.defer(function()
...
local _AstraV37Notify = Window.Notify
function Window:Notify(options)
    local before={}
    if self.NotificationHost then
        for _,child in ipairs(self.NotificationHost:GetChildren()) do before[child]=true end
    end
    local handle=_AstraV37Notify(self,options)
    if self.NotificationHost then
        task.defer(function()
            if self.Destroyed then return end
            for _,child in ipairs(self.NotificationHost:GetChildren()) do
                if child:IsA("Frame") and not before[child] then
                    local scale=v37EnsureScale(child,"AstraV37NotificationScale")
                    if v37EffectsEnabled(self) then
                        scale.Scale=0.94
                        tween(scale,0.20,{Scale=1},Enum.EasingStyle.Back,Enum.EasingDirection.Out)
                    end
                    break
                end
            end
...
local _AstraV37Dialog = Window.Dialog
function Window:Dialog(options)
    local before={}
    for _,child in ipairs(self.ScreenGui:GetChildren()) do before[child]=true end
    local handle=_AstraV37Dialog(self,options)
    task.defer(function()
        if self.Destroyed then return end
        for _,child in ipairs(self.ScreenGui:GetChildren()) do
            if not before[child] and child:IsA("TextButton") and child.ZIndex >= 190 then
                ...
                if card and v37EffectsEnabled(self) then
                    local scale=v37EnsureScale(card,"AstraV37DialogScale")
                    child.BackgroundTransparency=1
                    scale.Scale=0.93
                    card.BackgroundTransparency=0.08
                    tween(child,0.18,{BackgroundTransparency=0.38})
...
local _AstraV37SetTheme = Window.SetTheme
function Window:SetTheme(themePatch)
    _AstraV37SetTheme(self,themePatch)
    task.defer(function()
        if not self.Destroyed then v37RefreshThemeEffects(self) end
    end)
end

local _AstraV37SetMinimized = Window.SetMinimized
function Window:SetMinimized(state)
    if self.EffectGroup and v37EffectsEnabled(self) then
        if state then
            self.EffectGroup.GroupTransparency=0
            tween(self.EffectGroup,0.13,{GroupTransparency=1})
        else
            self.EffectGroup.GroupTransparency=1
        end
    end
    _AstraV37SetMinimized(self,state)
    if not state and self.EffectGroup and v37EffectsEnabled(self) then
        task.defer(function()
            if self.Destroyed or not self.EffectGroup.Parent then return end
            tween(self.EffectGroup,0.18,{GroupTransparency=0},Enum.EasingStyle.Quart,Enum.EasingDirection.Out)
        end)
    elseif self.EffectGroup and not v37EffectsEnabled(self) then
        self.EffectGroup.GroupTransparency=0
    end
end

local _AstraV37CreateWindow = AstraUI.CreateWindow
function AstraUI:CreateWindow(options)
    options=options or {}
    local window=_AstraV37CreateWindow(self,options)

    v37WrapWindowRoot(window)
    v37DecorateSearch(window)
    v37DecorateBrand(window)
    v37DecorateTopActions(window)
    v37RefreshThemeEffects(window)

    return window
end

AstraUI.Version = V37_VERSION
